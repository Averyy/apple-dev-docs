...
- `adGroupDeleted` (boolean): An indicator of whether the ad group is soft-deleted. You can use the `EQUALS` and `IN` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `adGroupId` (int64): The unique identifier for the ad group the search term belongs to. You can use the `EQUALS`, `IN`, and `STARTSWITH` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `adGroupName` (string): The name of the ad group, which is unique within the campaign. Responses don’t include deleted ad groups. You can use the `EQUALS`, `IN`, and `STARTSWITH` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
...
- `campaignId` (int64): The unique identifier for the campaign. You can use the `EQUALS`, `IN`, and `STARTSWITH` [`Selector`](selector.md) [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `deleted` (boolean): An indicator of whether the search term is soft-deleted. You can use the `EQUALS` and `IN` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `keyword` (string): The targeting or negative keywords. You can use the `EQUALS`, `IN`, and `STARTSWITH` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `keywordDisplayStatus` (string): The state of the keyword display operation. You can use the `EQUALS` and `IN` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `keywordId` (int64): The unique identifier for a keyword that belongs to an ad group.
- `keywordStatus` (string): The condition of the keyword. You can use the `EQUALS` selector [`Condition`](condition.md) operator with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
...
- `searchTermSource` (string): The source of the keyword to use as a search term. If `searchTermSource` is `AUTO`, `MatchType` is `AUTO`. If `searchTermSource` is `TARGETED`, `MatchType` is either `BROAD` or `EXACT`. - **`AUTO`**: The value to use to ensure Search Match automatically matches your ads.
- **`TARGETED`**: A bidded keyword. You can use the `EQUALS` and `IN` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
- `searchTermText` (string): The search terms to use for app searches. You can use the `EQUALS`, `IN`, and `STARTSWITH` selector [`Condition`](condition.md) operators with [`Get Search Term-Level Reports`](get-search-term-level-reports.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/apple_ads/reportingsearchterm)*