...
Typically you specify the optimizer to use with a training graph when creating the training graph, using [`init(graphObjects:lossLayer:optimizer:)`](mlctraininggraph/init(graphobjects:losslayer:optimizer:).md). The optimizer is then compiled in when you call [`compile(options:device:)`](mlctraininggraph/compile(options:device:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlctraininggraph/compileoptimizer(_:))*