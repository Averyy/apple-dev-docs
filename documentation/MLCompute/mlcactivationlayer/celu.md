...
This factory creates an activation descriptor using [`init(type:a:)`](mlcactivationdescriptor/init(type:a:).md), where `type =` [`MLCActivationType.celu`](mlcactivationtype/celu.md) and `a = 1.0`, and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/celu)*