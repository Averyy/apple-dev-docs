...
This factory creates an activation descriptor using [`init(type:a:)`](mlcactivationdescriptor/init(type:a:).md), where `type =` [`MLCActivationType.softShrink`](mlcactivationtype/softshrink.md) and `a = 0.5`, and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/softshrink)*