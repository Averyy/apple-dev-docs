...
The [`softPlus(beta:)`](mlcactivationlayer/softplus(beta:).md) factory type method creates an activation descriptor using [`init(type:a:b:)`](mlcactivationdescriptor/init(type:a:b:).md), where `type =` [`MLCActivationType.softPlus`](mlcactivationtype/softplus.md), `a = 1.0 / beta`, and `b = beta`, and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/softplus(beta:))*