...
The [`threshold(_:replacement:)`](mlcactivationlayer/threshold(_:replacement:).md) factory type method creates an activation descriptor using [`init(type:a:b:)`](mlcactivationdescriptor/init(type:a:b:).md), where `type =` [`MLCActivationType.threshold`](mlcactivationtype/threshold.md), `a = threshold`, and `b = replacement`, and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/threshold(_:replacement:))*