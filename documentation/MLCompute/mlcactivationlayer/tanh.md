...
This factory creates an activation descriptor using [`init(type:a:b:)`](mlcactivationdescriptor/init(type:a:b:).md), where `type =` [`MLCActivationType.tanh`](mlcactivationtype/tanh.md), `a = 1.0`, and `b = 1.0`, and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/tanh)*