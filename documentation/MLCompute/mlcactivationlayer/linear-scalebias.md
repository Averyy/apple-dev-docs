...
The [`linear(scale:bias:)`](mlcactivationlayer/linear(scale:bias:).md) factory type method creates an activation descriptor using [`init(type:a:b:)`](mlcactivationdescriptor/init(type:a:b:).md), where `type =` [`MLCActivationType.linear`](mlcactivationtype/linear.md), `a = scale`, and `b = bias`, and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/linear(scale:bias:))*