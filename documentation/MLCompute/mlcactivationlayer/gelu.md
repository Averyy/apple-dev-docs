...
This factory creates an activation descriptor using [`init(type:)`](mlcactivationdescriptor/init(type:).md), where `type =` [`MLCActivationType.gelu`](mlcactivationtype/gelu.md), and passes that descriptor to [`init(descriptor:)`](mlcactivationlayer/init(descriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mlcompute/mlcactivationlayer/gelu)*