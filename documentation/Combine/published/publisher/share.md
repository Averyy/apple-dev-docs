...
The following example uses a sequence publisher as a counter to publish three random numbers, generated by a [`map(_:)`](publisher/map(_:)-99evh.md) operator. It uses a [`share()`](published/publisher/share().md) operator to share the same random number to each of two subscribers. This example uses a [`delay(for:tolerance:scheduler:options:)`](published/publisher/delay(for:tolerance:scheduler:options:).md) operator only to prevent the first subscriber from exhausting the sequence publisher immediately; an asynchronous publisher wouldn’t need this.
...
Without the [`share()`](published/publisher/share().md) operator, stream 1 receives three random values, followed by stream 2 receiving three different random values.
...
*[View on Apple Developer](https://developer.apple.com/documentation/combine/published/publisher/share())*