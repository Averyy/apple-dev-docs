...
This method uses [`IOUSBGetNextDescriptor(_:_:)`](iousbgetnextdescriptor(_:_:).md), but returns `nil` if it finds another descriptor with a `bDescriptorType` field that matches the value for `parentDescripto`r’s `bDescriptorType`. Use `nil` for `currentDescriptor` to return the first descriptor after `parentDescriptor`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/iousbhost/iousbgetnextassociateddescriptor(_:_:_:))*