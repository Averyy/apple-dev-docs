# MusicSubscriptionOffer

**Framework**: MusicKit  
**Kind**: struct

A type for grouping other types for showing subscription offers for Apple Music.

**Availability**:
- iOS 15.0+
- iPadOS 15.0+
- Mac Catalyst 15.0+
- macOS 12.0+

## Declaration

```swift
struct MusicSubscriptionOffer
```

## Topics

### Structures
- [MusicSubscriptionOffer.Action](musicsubscriptionoffer/action.md)
  A representation of the entry point for the sheet with subscription offers for Apple Music.
- [MusicSubscriptionOffer.MessageIdentifier](musicsubscriptionoffer/messageidentifier.md)
  An identifier for the main message that the subscription offer sheet presents to the user.
- [MusicSubscriptionOffer.Options](musicsubscriptionoffer/options.md)
  Options for loading subscription offers for Apple Music.

## See Also

- [struct MusicSubscription](musicsubscription.md)
  A representation of the current state of the user’s subscription to Apple Music.


---

*[View on Apple Developer](https://developer.apple.com/documentation/musickit/musicsubscriptionoffer)*