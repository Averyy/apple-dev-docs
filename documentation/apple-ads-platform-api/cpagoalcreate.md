...
`cpaCap` is deprecated on `AdGroupCreate`. Use `BidStrategyCreate` with `bidStrategyType` set to `MAX_CONVERSIONS` instead of `CPAGoalCreate` to configure automated conversion-maximizing bidding.
...
*[View on Apple Developer](https://developer.apple.com/documentation/apple-ads-platform-api/cpagoalcreate)*