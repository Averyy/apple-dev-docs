...
`appDownloader` is one of only two `AdGroupTargeting` dimensions that support both `include` and `exclude`. Look up Adam IDs via [`Search for Apps`](searches-for-a-list-of-apps.md). Used with App Store campaigns. Uses the [`TargetingData`](targetingdata.md) `include`/`exclude` shape.
