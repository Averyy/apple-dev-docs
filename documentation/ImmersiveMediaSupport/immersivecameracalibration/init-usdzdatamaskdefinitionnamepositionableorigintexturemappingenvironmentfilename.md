# init(usdzData:maskDefinition:name:positionable:origin:textureMapping:environmentFilename:)
...
```swift
init(usdzData: Data, maskDefinition: ImmersiveDynamicMask, name: String = "", positionable: Bool = false, origin: ImmersiveCameraCalibration.CameraOrigin = .zero, textureMapping: ImmersiveCameraCalibration.CameraTextureMapping = .identity, environmentFilename: String? = nil)
```

## Parameters

- `usdzData`: The usdz data to be loaded and used as the camera calibration for each eye.
- `maskDefinition`: The dynamic mask definition describing the mask to be used for this calibration.
...
*[View on Apple Developer](https://developer.apple.com/documentation/immersivemediasupport/immersivecameracalibration/init(usdzdata:maskdefinition:name:positionable:origin:texturemapping:environmentfilename:))*