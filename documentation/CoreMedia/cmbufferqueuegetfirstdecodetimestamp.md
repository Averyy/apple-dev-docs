...
This API is is a faster alternative to [`CMBufferQueueIsEmpty(_:)`](cmbufferqueueisempty(_:).md),                but only gives the same answer if your queue is in decode order.                If the `getDecodeTimeStamp` callback is `NULL`, `kCMTimeInvalid` will                be returned.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coremedia/cmbufferqueuegetfirstdecodetimestamp(_:))*