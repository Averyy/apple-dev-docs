...
This callback is called once from [`CMBufferQueueGetFirstDecodeTimeStamp(_:)`](cmbufferqueuegetfirstdecodetimestamp(_:).md) and multiple times from [`CMBufferQueueGetMinDecodeTimeStamp(_:)`](cmbufferqueuegetmindecodetimestamp(_:).md).
...
This can be `NULL` ([`CMBufferQueueGetFirstDecodeTimeStamp(_:)`](cmbufferqueuegetfirstdecodetimestamp(_:).md) and [`CMBufferQueueGetMinDecodeTimeStamp(_:)`](cmbufferqueuegetmindecodetimestamp(_:).md) will return `kCMTimeInvalid`).
...
*[View on Apple Developer](https://developer.apple.com/documentation/coremedia/cmbuffercallbacks/getdecodetimestamp)*