...
User interface classes must implement this method to return descriptions for all actions returned from [`accessibilityActionNames()`](nsobject-swift.class/accessibilityactionnames().md). A button, for example, might return the string “press” for the `NSAccessibilityPressAction` action. Subclasses should invoke the superclass’s implementation, if it exists, to obtain the descriptions of any inherited actions.
...
*[View on Apple Developer](https://developer.apple.com/documentation/objectivec/nsobject-swift.class/accessibilityactiondescription(_:))*