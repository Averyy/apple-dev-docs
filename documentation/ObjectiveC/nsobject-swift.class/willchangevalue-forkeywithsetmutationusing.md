...
> ❗ **Important**:  After the values have been changed, a corresponding [`didChangeValue(forKey:withSetMutation:using:)`](nsobject-swift.class/didchangevalue(forkey:withsetmutation:using:).md) must be invoked with the same parameters.

 After the values have been changed, a corresponding [`didChangeValue(forKey:withSetMutation:using:)`](nsobject-swift.class/didchangevalue(forkey:withsetmutation:using:).md) must be invoked with the same parameters.
...
- `objects`: The objects that are involved in the change (see  ).
...
*[View on Apple Developer](https://developer.apple.com/documentation/objectivec/nsobject-swift.class/willchangevalue(forkey:withsetmutation:using:))*