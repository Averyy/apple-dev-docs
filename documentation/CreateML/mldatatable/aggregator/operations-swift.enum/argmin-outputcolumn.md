...
To find out which person gave the lowest rating for any particular beverage, use the `argmin` operation with the `"userName"` string. Then use the [`group(columnsNamed:aggregators:)`](mldatatable/group(columnsnamed:aggregators:).md) function group the `"drink"` column.
...
- [MLDataTable.Aggregator.Operations.argmax(outputColumn:)](mldatatable/aggregator/operations-swift.enum/argmax(outputcolumn:).md)
...
*[View on Apple Developer](https://developer.apple.com/documentation/createml/mldatatable/aggregator/operations-swift.enum/argmin(outputcolumn:))*