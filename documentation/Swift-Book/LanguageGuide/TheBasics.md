...
url: https://github.com/swiftlang/swift-book/blob/main/TSPL.docc/LanguageGuide/TheBasics.md
---
...
see [Constant Declaration](Declarations.md#Constant-Declaration).
...
> as described in [Type Safety and Type Inference](TheBasics.md#Type-Safety-and-Type-Inference).
...
see [Default Parameter Values](Functions.md#Default-Parameter-Values).
...
> are described in [String Interpolation](StringsAndCharacters.md#String-Interpolation).
...
as described in [Overflow Operators](AdvancedOperators.md#Overflow-Operators).
...
> and matches integer type inference, as described in [Type Safety and Type Inference](TheBasics.md#Type-Safety-and-Type-Inference).
...
as described in [Integers](TheBasics.md#Integers).
...
is covered in [Extensions](Extensions.md).
...
Conditional statements such as the `if` statement are covered in more detail in [Control Flow](ControlFlow.md).
...
Comparisons like `i == 1` are discussed in [Basic Operators](BasicOperators.md).
...
For more information, see [Functions with Multiple Return Values](Functions.md#Functions-with-Multiple-Return-Values).
...
> For more information, see [Classes And Structures](ClassesAndStructures.md).
...
- Propagate the `nil` value,
  by returning `nil`
  or using the `?.` operator described in [Optional Chaining](OptionalChaining.md).
...
see [Control Flow](ControlFlow.md).
...
the [Optionals](TheBasics.md#Optionals) section
...
as described in [Early Exit](ControlFlow.md#Early-Exit).
...
see [Nil Coalescing Operator](BasicOperators.md#Nil-Coalescing-Operator).
...
see [Assertions and Preconditions](TheBasics.md#Assertions-and-Preconditions).
...
as described in [Unowned References and Implicitly Unwrapped Optional Properties](AutomaticReferenceCounting.md#Unowned-References-and-Implicitly-Unwrapped-Optional-Properties).
...
described above in [Type Safety and Type Inference](TheBasics.md#Type-Safety-and-Type-Inference),
...
discussed in [Error Handling](TheBasics.md#Error-Handling)
...
within the code.
Unlike the error conditions discussed in [Error Handling](TheBasics.md#Error-Handling) above,
