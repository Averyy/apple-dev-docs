...
url: https://github.com/swiftlang/swift-book/blob/main/TSPL.docc/LanguageGuide/ControlFlow.md
---
...
For more about arrays and dictionaries, see [Collection Types](CollectionTypes.md).
...
For more about ranges, see [Range Operators](BasicOperators.md#Range-Operators).
...
as described in [Type Annotations](TheBasics.md#Type-Annotations).
...
For information about working with errors, see [Error Handling](ErrorHandling.md).
...
> For details, see [Break in a Switch Statement](ControlFlow.md#Break-in-a-Switch-Statement).
...
see [Compound Cases](ControlFlow.md#Compound-Cases).

> Note: To explicitly fall through at the end of a particular `switch` case,
> use the `fallthrough` keyword,
> as described in [Fallthrough](ControlFlow.md#Fallthrough).
...
as described in [Associated Values](Enumerations.md#Associated-Values).
...
The `return` statement is described in [Functions](Functions.md),
and the `throw` statement is described in [Propagating Errors Using Throwing Functions](ErrorHandling.md#Propagating-Errors-Using-Throwing-Functions).
...
see [Specifying Cleanup Actions](ErrorHandling.md#Specifying-Cleanup-Actions).
...
for the full list, see [Declaration Attributes](Attributes.md#Declaration-Attributes).
