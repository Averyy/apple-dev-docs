# DOMCharacterData

**Framework**: Webkit  
**Kind**: class

**Availability**:
- macOS 10.4+

## Declaration

```swift
class DOMCharacterData
```

## Topics

### Instance Properties
- [var data: String!](domcharacterdata/data.md)
- [var length: UInt32](domcharacterdata/length.md)
### Instance Methods
- [func appendData(String!)](domcharacterdata/appenddata(_:).md)
- [func deleteData(UInt32, length: UInt32)](domcharacterdata/deletedata(_:length:).md)
- [func insertData(UInt32, data: String!)](domcharacterdata/insertdata(_:data:).md)
- [func replaceData(UInt32, length: UInt32, data: String!)](domcharacterdata/replacedata(_:length:data:).md)
- [func substringData(UInt32, length: UInt32) -> String!](domcharacterdata/substringdata(_:length:).md)

## Relationships

### Inherits From
- [DOMNode](domnode.md)
### Inherited By
- [DOMComment](domcomment.md)
- [DOMProcessingInstruction](domprocessinginstruction.md)
- [DOMText](domtext.md)
### Conforms To
- [CVarArg](../Swift/CVarArg.md)
- [CustomDebugStringConvertible](../Swift/CustomDebugStringConvertible.md)
- [CustomStringConvertible](../Swift/CustomStringConvertible.md)
- [DOMEventTarget](domeventtarget.md)
- [Equatable](../Swift/Equatable.md)
- [Hashable](../Swift/Hashable.md)
- [NSCopying](../Foundation/NSCopying.md)
- [NSObjectProtocol](../ObjectiveC/NSObjectProtocol.md)

## See Also

- [class DOMAbstractView](domabstractview.md)
- [class DOMAttr](domattr.md)
- [class DOMBlob](domblob.md)
- [class DOMCDATASection](domcdatasection.md)
- [class DOMComment](domcomment.md)
- [class DOMCounter](domcounter.md)
- [class DOMCSSCharsetRule](domcsscharsetrule.md)
- [class DOMCSSFontFaceRule](domcssfontfacerule.md)
- [class DOMCSSImportRule](domcssimportrule.md)
- [class DOMCSSMediaRule](domcssmediarule.md)
- [class DOMCSSPageRule](domcsspagerule.md)
- [class DOMCSSPrimitiveValue](domcssprimitivevalue.md)
- [class DOMCSSRule](domcssrule.md)
- [class DOMCSSRuleList](domcssrulelist.md)
- [class DOMCSSStyleDeclaration](domcssstyledeclaration.md)


---

*[View on Apple Developer](https://developer.apple.com/documentation/webkit/domcharacterdata)*