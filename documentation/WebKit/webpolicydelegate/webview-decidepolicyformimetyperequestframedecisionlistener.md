...
If you do not implement this method, the default behavior is used. The listener is told to ignore the MIME type unless `webView` specifies it can handle the type in its [`canShowMIMEType(_:)`](webview/canshowmimetype(_:).md) method.
