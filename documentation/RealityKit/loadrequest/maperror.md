...
The following example uses a `Publisher/tryMap(_:)` operator to divide `1` by each element produced by a sequence publisher. When the publisher produces a `0`, the `Publisher/tryMap(_:)` fails with a `DivisionByZeroError`. The `Publisher/mapError(_:)` operator converts this into a `MyGenericError`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/loadrequest/maperror(_:))*