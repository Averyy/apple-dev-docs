...
Combine’s `Publisher/filter(_:)` operator performs an operation similar to that of [`filter(_:)`](https://developer.apple.com/documentation/Swift/Sequence/filter(_:)-5y9d2) in the Swift Standard Library: it uses a closure to test each element to determine whether to republish the element to the downstream subscriber.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/loadrequest/filter(_:))*