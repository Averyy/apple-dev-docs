...
In this example a `PassthroughSubject` publishes strings, but its downstream [`tryMap(_:)`](scene/publisher/trymap(_:).md) operator throws an error. This sends the error downstream as a `Subscribers/Completion/failure(_:)`. The [`breakpointOnError()`](scene/publisher/breakpointonerror().md) operator receives this completion and stops the app in the debugger.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/breakpointonerror())*