...
Use [`combineLatest(_:_:_:_:)`](scene/publisher/combinelatest(_:_:_:_:).md) when you need to combine the current and 3 additional publishers and transform the values using a closure in which you specify the published elements, to publish a new element.
...
In the example below, as [`combineLatest(_:_:_:_:)`](scene/publisher/combinelatest(_:_:_:_:).md) receives the most-recent values published by four publishers, multiplies them together, and republishes the result:
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/combinelatest(_:_:_:_:))*