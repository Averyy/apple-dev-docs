...
Use [`tryCompactMap(_:)`](scene/publisher/trycompactmap(_:).md) to remove `nil` elements from a publisher’s stream based on an error-throwing closure you provide. If the closure throws an error, the publisher cancels the upstream publisher and sends the thrown error to the downstream subscriber as a [`Scene.Publisher.Failure`](scene/publisher/failure.md).

The following example uses an array of numbers as the source for a collection-based publisher. A [`tryCompactMap(_:)`](scene/publisher/trycompactmap(_:).md) operator consumes each integer from the publisher and uses a dictionary to transform the numbers from its Arabic to Roman numerals, as an optional [`String`](https://developer.apple.com/documentation/Swift/String).

If the closure called by [`tryCompactMap(_:)`](scene/publisher/trycompactmap(_:).md) fails to look up a Roman numeral, it returns the optional String `(unknown)`.

If the closure called by [`tryCompactMap(_:)`](scene/publisher/trycompactmap(_:).md) determines the input is `0`, it throws an error. The [`tryCompactMap(_:)`](scene/publisher/trycompactmap(_:).md) operator catches this error and stops publishing, sending a `Subscribers/Completion/failure(_:)` that wraps the error.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/trycompactmap(_:))*