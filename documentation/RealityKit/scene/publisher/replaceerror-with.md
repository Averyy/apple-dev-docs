...
In the example below, a publisher of strings fails with a `MyError` instance, which sends a failure completion downstream. The [`replaceError(with:)`](scene/publisher/replaceerror(with:).md) operator handles the failure by publishing the string `(replacement element)` and completing normally.
...
This [`replaceError(with:)`](scene/publisher/replaceerror(with:).md) functionality is useful when you want to handle an error by sending a single replacement element and end the stream. Use [`catch(_:)`](scene/publisher/catch(_:).md) to recover from an error and provide a replacement publisher to continue providing elements to the downstream subscriber.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/replaceerror(with:))*