...
Use [`tryRemoveDuplicates(by:)`](scene/publisher/tryremoveduplicates(by:).md) to remove repeating elements from an upstream publisher based upon the evaluation of elements using an error-throwing closure you provide. If your closure throws an error, the publisher terminates with the error.

In the example below, the closure provided to [`tryRemoveDuplicates(by:)`](scene/publisher/tryremoveduplicates(by:).md) returns `true` when two consecutive elements are equal, thereby filtering out `0`, `1`, `2`, and `3`. However, the closure throws an error when it encounters `4`. The publisher then terminates with this error.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/tryremoveduplicates(by:))*