...
In the following example, the [`map(_:_:_:)`](scene/publisher/map(_:_:_:).md) operator uses the Swift key path syntax to access the `die1`, `die2`, and `die3` members of the `DiceRoll` structure published by the `Just` publisher.
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/map(_:_:_:))*