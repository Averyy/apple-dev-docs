...
Use [`prefix(while:)`](scene/publisher/prefix(while:).md) to emit values while elements from the upstream publisher meet a condition you specify. The publisher finishes when the closure returns `false`.

In the example below, the [`prefix(while:)`](scene/publisher/prefix(while:).md) operator emits values while the element it receives is less than five:
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/prefix(while:))*