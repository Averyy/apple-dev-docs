...
Use `Publisher/combineLatest(_:_:_:)-48buc` when you want the downstream subscriber to receive a tuple of the most-recent element from multiple publishers when any of them emit a value. To combine elements from multiple publishers, use `Publisher/zip(_:_:_:)-16rcy` instead. To receive just the most-recent element from multiple publishers rather than tuples, use [`merge(with:_:_:)`](scene/publisher/merge(with:_:_:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/scene/publisher/combinelatest(_:_:_:)-8tlvp)*