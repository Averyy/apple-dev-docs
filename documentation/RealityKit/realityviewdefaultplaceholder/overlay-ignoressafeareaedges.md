...
Use this modifier to layer a type that conforms to the `ShapeStyle` protocol, like a `Color`, [`Material`](material.md), or `HierarchicalShapeStyle`, in front of a view. For example, you can overlay the `ShapeStyle/ultraThinMaterial` over a `Circle`:
...
*[View on Apple Developer](https://developer.apple.com/documentation/realitykit/realityviewdefaultplaceholder/overlay(_:ignoressafeareaedges:))*