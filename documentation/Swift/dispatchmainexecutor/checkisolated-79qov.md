...
During executor comparison, the Swift concurrency runtime attempts to compare current and expected executors in a few ways (including “complex” equality between executors (see [`isSameExclusiveExecutionContext(other:)`](dispatchmainexecutor/issameexclusiveexecutioncontext(other:).md)), and if all those checks fail, this method is invoked on the expected executor.
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/dispatchmainexecutor/checkisolated()-79qov)*