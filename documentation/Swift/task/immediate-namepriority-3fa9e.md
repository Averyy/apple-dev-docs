...
```swift
@discardableResult
static func immediate(name: String? = nil, priority: TaskPriority? = nil, _ operation: @escaping @isolated(any) () async throws -> Success) -> Task<Success, any Error>
```
...
Other than the execution semantics discussed above, the created task is semantically equivalent to its basic version which can be created using `Task/init`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/task/immediate(name:priority:_:)-3fa9e)*