...
In this example, the output shows that the [`print(_:to:)`](result/publisher-swift.struct/print(_:to:).md) operator receives each random value only one time, and then sends the value to both subscribers.
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/result/publisher-swift.struct/multicast(_:))*