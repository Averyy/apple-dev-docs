...
Use the [`assign(to:on:)`](result/publisher-swift.struct/assign(to:on:).md) subscriber when you want to set a given property each time a publisher produces a value.

In this example, the [`assign(to:on:)`](result/publisher-swift.struct/assign(to:on:).md) sets the value of the `anInt` property on an instance of `MyClass`:
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/result/publisher-swift.struct/assign(to:on:))*