...
In this example a `PassthroughSubject` publishes strings, but its downstream [`tryMap(_:)`](optional/publisher-swift.struct/trymap(_:).md) operator throws an error. This sends the error downstream as a `Subscribers/Completion/failure(_:)`. The [`breakpointOnError()`](optional/publisher-swift.struct/breakpointonerror().md) operator receives this completion and stops the app in the debugger.
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/optional/publisher-swift.struct/breakpointonerror())*