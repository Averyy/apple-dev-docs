...
Use [`decode(type:decoder:)`](optional/publisher-swift.struct/decode(type:decoder:).md) with a [`JSONDecoder`](https://developer.apple.com/documentation/Foundation/JSONDecoder) (or a [`PropertyListDecoder`](https://developer.apple.com/documentation/Foundation/PropertyListDecoder) for property lists) to decode data received from a [`URLSession.DataTaskPublisher`](https://developer.apple.com/documentation/Foundation/URLSession/DataTaskPublisher) or other data source using the [`Decodable`](Decodable.md) protocol.

In this example, a `PassthroughSubject` publishes a JSON string. The JSON decoder parses the string, converting its fields according to the [`Decodable`](Decodable.md) protocol implemented by `Article`, and successfully populating a new `Article`. The `Publishers/Decode` publisher then publishes the `Article` to the downstream. If a decoding operation fails, which happens in the case of missing or malformed data in the source JSON string, the stream terminates and passes the error to the downstream subscriber.
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/optional/publisher-swift.struct/decode(type:decoder:))*