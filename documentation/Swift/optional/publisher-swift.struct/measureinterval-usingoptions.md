...
Use [`measureInterval(using:options:)`](optional/publisher-swift.struct/measureinterval(using:options:).md) to measure the time between events delivered from an upstream publisher.

In the example below, a 1-second [`Timer`](https://developer.apple.com/documentation/Foundation/Timer) is used as the data source for an event publisher; the [`measureInterval(using:options:)`](optional/publisher-swift.struct/measureinterval(using:options:).md) operator reports the elapsed time between the reception of events on the main run loop:
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/optional/publisher-swift.struct/measureinterval(using:options:))*