...
Use [`timeout(_:scheduler:options:customError:)`](optional/publisher-swift.struct/timeout(_:scheduler:options:customerror:).md) to terminate a publisher if an element isn’t delivered within a timeout interval you specify.

In the example below, a `PassthroughSubject` publishes [`String`](String.md) elements and is configured to time out if no new elements are received within its `TIME_OUT` window of 5 seconds. A single value is published after the specified 2-second `WAIT_TIME`, after which no more elements are available; the publisher then times out and completes normally.
...
*[View on Apple Developer](https://developer.apple.com/documentation/swift/optional/publisher-swift.struct/timeout(_:scheduler:options:customerror:))*