...
Typically an `NSDocument` subclass overrides [`windowNibName`](nsdocument/windownibname.md) or [`makeWindowControllers()`](nsdocument/makewindowcontrollers().md), but not both. If [`windowNibName`](nsdocument/windownibname.md) is overridden, the default implementation of [`makeWindowControllers()`](nsdocument/makewindowcontrollers().md) will load the named nib file, making the `NSDocument` object the nib file’s owner. In that case, you can override [`windowControllerDidLoadNib(_:)`](nsdocument/windowcontrollerdidloadnib(_:).md) and do custom processing after the nib file is loaded.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsdocument/windowcontrollerdidloadnib(_:))*