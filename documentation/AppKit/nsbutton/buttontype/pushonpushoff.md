...
When [`state`](nsbutton/state.md) is [`on`](nscontrol/statevalue/on.md), the button appears illuminated. If the button has borders, it may also appear recessed. A second click returns the button’s [`state`](nsbutton/state.md) to [`off`](nscontrol/statevalue/off.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsbutton/buttontype/pushonpushoff)*