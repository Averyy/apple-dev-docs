...
New cells are created if needed with [`makeCell(atRow:column:)`](nsmatrix/makecell(atrow:column:).md). This method redraws the receiver. Your code may need to send [`sizeToCells()`](nsmatrix/sizetocells().md) after sending this method to resize the receiver to fit the newly added cells.

If the number of rows or columns in the receiver has been changed with [`renewRows(_:columns:)`](nsmatrix/renewrows(_:columns:).md), then new cells are created only if they’re needed. This fact allows you to grow and shrink an [`NSMatrix`](nsmatrix.md) without repeatedly creating and freeing the cells.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsmatrix/insertrow(_:))*