...
The minimum size constraint is enforced for resizing by the user as well as for the `setFrame...` methods other than [`setFrame(_:display:)`](nswindow/setframe(_:display:).md) and [`setFrame(_:display:animate:)`](nswindow/setframe(_:display:animate:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nswindow/minsize)*