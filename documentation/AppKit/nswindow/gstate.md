...
This graphics state is used by default for all `NSView` objects in the window’s view hierarchy, but individual views can be made to use their own with the `NSView` method [`allocateGState()`](nsview/allocategstate().md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nswindow/gstate())*