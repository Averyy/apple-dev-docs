...
The returned point can be passed to a subsequent invocation of [`cascadeTopLeft(from:)`](nswindow/cascadetopleft(from:).md) to position the next window so the title bars of both windows are fully visible.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nswindow/cascadetopleft(from:))*