...
When `true`, the text attachment tries to use a text attachment view returned by [`viewProvider(for:location:textContainer:)`](nstextattachmentlayout/viewprovider(for:location:textcontainer:).md). Default is `true`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nstextattachment/allowstextattachmentview)*