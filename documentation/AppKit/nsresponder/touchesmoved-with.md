...
The system sends the to the view under the touch in the key window. To get the set of touches that moved for this view (or descendants of this view) call [`touches(matching:in:)`](nsevent/touches(matching:in:).md) on `event` and pass [`moved`](nstouch/phase-swift.struct/moved.md) for the phase.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsresponder/touchesmoved(with:))*