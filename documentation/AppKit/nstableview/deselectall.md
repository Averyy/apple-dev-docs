...
Posts [`selectionDidChangeNotification`](nstableview/selectiondidchangenotification.md) to the default notification center if the selection does in fact change.

As a target-action method, [`deselectAll(_:)`](nstableview/deselectall(_:).md) checks with the delegate before changing the selection, using [`selectionShouldChange(in:)`](nstableviewdelegate/selectionshouldchange(in:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nstableview/deselectall(_:))*