...
The actual means of presentation of the potential completions is determined by the [`complete(_:)`](nstextview/complete(_:).md) method of `NSTextView`.
...
- `words`: An array of   objects containing the potential completions. The completion strings are listed in their order of preference in the array.
- `charRange`: The range of characters the user has already typed.
- `index`: On input, an integer variable with the default value of  . On output, you can set this value to an index in the returned array indicating which item should be selected initially. Set the value to   to indicate there should not be an initial selection.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nscontroltexteditingdelegate/control(_:textview:completions:forpartialwordrange:indexofselecteditem:))*