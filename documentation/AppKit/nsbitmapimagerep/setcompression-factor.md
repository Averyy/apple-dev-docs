...
When an [`NSBitmapImageRep`](nsbitmapimagerep.md) is created, the instance stores the compression type and factor for the source data. The [`tiffRepresentation`](nsbitmapimagerep/tiffrepresentation.md) property and [`tiffRepresentationOfImageReps(in:)`](nsbitmapimagerep/tiffrepresentationofimagereps(in:).md) class method try to use the stored compression type and factor. Use this method to change the compression type and factor.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsbitmapimagerep/setcompression(_:factor:))*