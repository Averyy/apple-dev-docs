...
This method invokes [`stateImageRect(forBounds:)`](nsmenuitemcell/stateimagerect(forbounds:).md), passing it `cellFrame`, to calculate the rectangle in which to draw the state image. This method is invoked by the cell’s `drawWithFrame:` method. You should not need to invoke it directly. Subclasses may override this method to control the drawing of the state image.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsmenuitemcell/drawstateimage(withframe:in:))*