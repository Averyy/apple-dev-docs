...
Call this method after a previous call to the [`lockFocus()`](nsview/lockfocus().md), [`lockFocusIfCanDraw()`](nsview/lockfocusifcandraw().md), or [`lockFocusIfCanDraw(in:)`](nsview/lockfocusifcandraw(in:).md) method of this view object. Doing so releases focus from the current view and returns it back to the previously focused view, if any. This method raises an `NSInvalidArgumentException` if the current view does not have the focus.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsview/unlockfocus())*