...
A view object’s graphics state object is recreated from scratch whenever the view is focused, unless the [`allocateGState()`](nsview/allocategstate().md) method has been invoked. So if the view hasn’t been focused or hasn’t received the [`allocateGState()`](nsview/allocategstate().md) message, this method returns 0.

Although applications rarely need to use the value returned by [`gState()`](nsview/gstate().md), it can be passed to the few methods that take an object identifier as a parameter.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appkit/nsview/gstate())*