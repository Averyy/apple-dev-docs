...
Instances of the [`NSTouchBar`](nstouchbar.md) class employ gesture recognizers and take advantage of macOS 10.12.1 event enhancements. Because of the physical geometry of the Touch Bar, touch events passed to gesture recognizers have only a meaningful , or horizontal, component.
...
For Touch Bar design guidance, read [`Human Interface Guidelines`](https://developer.apple.comhttps://developer.apple.com/design/human-interface-guidelines/inputs/touch-bar/).

##### Bar Objects

You can think of an [`NSTouchBar`](nstouchbar.md) object (or ), with its array of [`NSTouchBarItem`](nstouchbaritem.md) objects (or ), as analogous to a window toolbar with its toolbar items, or a menu with its menu items.
...
- Implement the [`makeTouchBar()`](nsresponder/maketouchbar().md) delegate method from the [`NSTouchBarProvider`](nstouchbarprovider.md) protocol.

The built-in responder classes conform to the [`NSTouchBarProvider`](nstouchbarprovider.md) protocol and support key-value observing (KVO), both of which are used and required by the [`NSTouchBar`](nstouchbar.md) infrastructure. In the context of Touch Bar support, a responder instance can also be called a .

The following code shows an example implementation of the [`makeTouchBar()`](nsresponder/maketouchbar().md) delegate method. In this code snippet, you can see some statements related to bar customization.
...
- The [`NSTouchBarDelegate`](nstouchbardelegate.md) protocol, and its [`touchBar(_:makeItemForIdentifier:)`](nstouchbardelegate/touchbar(_:makeitemforidentifier:).md) delegate method, give your app a way to create items on-demand. Use this option when it makes more sense in terms of resource usage and reflecting dynamic state.
...
, specified in a bar’s [`defaultItemIdentifiers`](nstouchbar/defaultitemidentifiers.md) property. Always specify this property for an [`NSTouchBar`](nstouchbar.md) object, even if you elect to make the bar noncustomizable. The system:
...
, specified in a bar’s [`customizationRequiredItemIdentifiers`](nstouchbar/customizationrequireditemidentifiers.md) property. Configure this property at your discretion, depending on the design of your app. The user can’t remove from the bar any of the items you specify in this list.
...
- If you configure a bar as  customizable, and give it a group item whose bar you configure as customizable, then the [`groupTouchBar`](nsgrouptouchbaritem/grouptouchbar.md) bar’s items appear in the customization UI as individual items. During customization, a user can manipulate each item separately: If an item from the [`groupTouchBar`](nsgrouptouchbaritem/grouptouchbar.md) bar is visible in the Touch Bar, the user can remove it or can rearrange its position individually among the other items in the [`groupTouchBar`](nsgrouptouchbaritem/grouptouchbar.md) bar; if an item from the [`groupTouchBar`](nsgrouptouchbaritem/grouptouchbar.md) bar is instead visible in the customization UI, the user can add it back to the Touch Bar, individually, placing it anywhere within the [`groupTouchBar`](nsgrouptouchbaritem/grouptouchbar.md) bar that owns it.
 ...
 (an instance of the [`NSPopoverTouchBarItem`](nspopovertouchbaritem.md) class) has two bars: one bar you specify in its [`popoverTouchBar`](nspopovertouchbaritem/popovertouchbar.md) property and a second, optional bar you can specify in its [`pressAndHoldTouchBar`](nspopovertouchbaritem/pressandholdtouchbar.md) property. Here are some examples of how customization for popover items works in practice:

- If you configure a bar as customizable, and give it a popover item whose [`popoverTouchBar`](nspopovertouchbaritem/popovertouchbar.md) bar you configure as  customizable, the [`popoverTouchBar`](nspopovertouchbaritem/popovertouchbar.md) bar never appears in the customization UI. If the user invokes the customization UI when the (noncustomizable) popover item itself (not the button’s associated [`popoverTouchBar`](nspopovertouchbaritem/popovertouchbar.md) bar) is visible in the Touch Bar, the customization UI lets the user rearrange the position of the popover item relative to the other items in the containing bar. If, on the other hand, the user invokes the customization UI when the (noncustomizable) [`popoverTouchBar`](nspopovertouchbaritem/popovertouchbar.md) bar is visible in the Touch Bar, the system dismisses the popover bar and shows, in the customization UI, customization options for the bar that contains the popover item.
...
The Touch Bar can show one bar nested within another, as described in [`Composition and nesting`](nstouchbar#Composition-and-nesting.md).
...
-  The [`NSScrubber`](nsscrubber.md) class, along with related APIs, provide a way for you to add a flexible, horizontally-oriented picker to a custom item (an instance of the [`NSCustomTouchBarItem`](nscustomtouchbaritem.md) class).
...
- The [`NSGestureRecognizer`](nsgesturerecognizer.md) abstract class is enhanced with a set of methods that let you implement responses to touch events: [`touchesBegan(with:)`](nsgesturerecognizer/touchesbegan(with:).md), [`touchesCancelled(with:)`](nsgesturerecognizer/touchescancelled(with:).md), [`touchesEnded(with:)`](nsgesturerecognizer/touchesended(with:).md), and [`touchesMoved(with:)`](nsgesturerecognizer/touchesmoved(with:).md).
...
- When you use an [`NSCandidateListTouchBarItem`](nscandidatelisttouchbaritem.md) object, you can use the [`requestCandidates(forSelectedRange:in:types:options:inSpellDocumentWithTag:completionHandler:)`](nsspellchecker/requestcandidates(forselectedrange:in:types:options:inspelldocumentwithtag:completionhandler:).md) method. This method provides a completion handler that you can use to filter or otherwise manage the candidate text.
-  AppKit adds many new template images for you to use in your [`NSTouchBarItem`](nstouchbaritem.md) objects. A few examples of these images are: [`touchBarAddTemplateName`](nsimage/touchbaraddtemplatename.md), [`touchBarComposeTemplateName`](nsimage/touchbarcomposetemplatename.md), [`touchBarGoBackTemplateName`](nsimage/touchbargobacktemplatename.md), [`touchBarGoForwardTemplateName`](nsimage/touchbargoforwardtemplatename.md), and `NSImageNameTouchBarHomeTemplate`. Always use templates for images in your items: they respond automatically to system white-point changes. Note that these images are exclusively for use in the Touch Bar and  in onscreen windows. For a complete list of these template images, see [`NSTouchBarItem`](nstouchbaritem.md).
-  If your UI for a popover item needs more horizontal space, you can use a scroll view (an instance of the [`NSScrollView`](nsscrollview.md) class). , in this case, enable the popover item’s press-and-hold option, because doing so interferes with scrolling.
-  You can group bar items by using an instance of the [`NSStackView`](nsstackview.md) class. However, doing so loses system support for spacing. When you instead place items into a group item (an instance of the [`NSGroupTouchBarItem`](nsgrouptouchbaritem.md) class), the system:
...
- When a user is interacting with the Touch Bar  watching the Touch Bar, such as to control audio, ensure that your app gives priority to updating the Touch Bar.
...
### Creating a bar
- [init()](nstouchbar/init.md)
  Creates a Touch Bar object.
- [init?(coder: NSCoder)](nstouchbar/init(coder:).md)
