# HumanHandPoseObservation.JointName

**Framework**: Vision  
**Kind**: enum

The supported joint names for the hand pose.

**Availability**:
- iOS 18.0+
- iPadOS 18.0+
- macOS 15.0+
- tvOS 18.0+
- visionOS 2.0+

## Declaration

```swift
enum JointName
```

## Topics

### Getting the thumb finger joint name
- [HumanHandPoseObservation.JointName.thumbCMC](humanhandposeobservation/jointname/thumbcmc.md)
  A joint name that represents the thumb’s carpometacarpal (CMC) joint.
- [HumanHandPoseObservation.JointName.thumbIP](humanhandposeobservation/jointname/thumbip.md)
  A joint name that represents the thumb’s interphalangeal (IP) joint.
- [HumanHandPoseObservation.JointName.thumbMP](humanhandposeobservation/jointname/thumbmp.md)
  A joint name that represents the thumb’s metacarpophalangeal (MP) joint.
- [HumanHandPoseObservation.JointName.thumbTip](humanhandposeobservation/jointname/thumbtip.md)
  A joint name that represents the tip of the thumb.
### Getting the index finger joint name
- [HumanHandPoseObservation.JointName.indexDIP](humanhandposeobservation/jointname/indexdip.md)
  A joint name that represents the index finger’s distal interphalangeal joint.
- [HumanHandPoseObservation.JointName.indexMCP](humanhandposeobservation/jointname/indexmcp.md)
  A joint name that represents the index finger’s metacarpophalangeal joint.
- [HumanHandPoseObservation.JointName.indexPIP](humanhandposeobservation/jointname/indexpip.md)
  A joint name that represents the index finger’s proximal interphalangeal joint.
- [HumanHandPoseObservation.JointName.indexTip](humanhandposeobservation/jointname/indextip.md)
  A joint name that represents the tip of the index finger.
### Getting the middle finger joint name
- [HumanHandPoseObservation.JointName.middleDIP](humanhandposeobservation/jointname/middledip.md)
  A joint name that represents the middle finger’s distal interphalangeal joint.
- [HumanHandPoseObservation.JointName.middleMCP](humanhandposeobservation/jointname/middlemcp.md)
  A joint name that represents the middle finger’s metacarpophalangeal joint.
- [HumanHandPoseObservation.JointName.middlePIP](humanhandposeobservation/jointname/middlepip.md)
  A joint name that represents the middle finger’s proximal interphalangeal joint.
- [HumanHandPoseObservation.JointName.middleTip](humanhandposeobservation/jointname/middletip.md)
  A joint name that represents the tip of the middle finger.
### Getting the ring finger joint name
- [HumanHandPoseObservation.JointName.ringDIP](humanhandposeobservation/jointname/ringdip.md)
  A joint name that represents the ring finger’s distal interphalangeal joint.
- [HumanHandPoseObservation.JointName.ringMCP](humanhandposeobservation/jointname/ringmcp.md)
  A joint name that represents the ring finger’s metacarpophalangeal joint.
- [HumanHandPoseObservation.JointName.ringPIP](humanhandposeobservation/jointname/ringpip.md)
  A joint name that represents the ring finger’s proximal interphalangeal joint.
- [HumanHandPoseObservation.JointName.ringTip](humanhandposeobservation/jointname/ringtip.md)
  A joint name that represents the tip of the ring finger.
### Getting the little finger joint name
- [HumanHandPoseObservation.JointName.littleDIP](humanhandposeobservation/jointname/littledip.md)
  A joint name that represents the little finger’s distal interphalangeal joint.
- [HumanHandPoseObservation.JointName.littleMCP](humanhandposeobservation/jointname/littlemcp.md)
  A joint name that represents the little finger’s metacarpophalangeal joint.
- [HumanHandPoseObservation.JointName.littlePIP](humanhandposeobservation/jointname/littlepip.md)
  A joint name that represents the little finger’s proximal interphalangeal joint.
- [HumanHandPoseObservation.JointName.littleTip](humanhandposeobservation/jointname/littletip.md)
  A joint name that represents the tip of the little finger.
### Getting the wrist joint name
- [HumanHandPoseObservation.JointName.wrist](humanhandposeobservation/jointname/wrist.md)
  A joint name that represents the wrist.

## Relationships

### Conforms To
- [Copyable](../Swift/Copyable.md)
- [Decodable](../Swift/Decodable.md)
- [Encodable](../Swift/Encodable.md)
- [Equatable](../Swift/Equatable.md)
- [Hashable](../Swift/Hashable.md)
- [RawRepresentable](../Swift/RawRepresentable.md)
- [Sendable](../Swift/Sendable.md)
- [SendableMetatype](../Swift/SendableMetatype.md)

## See Also

- [HumanHandPoseObservation.JointsGroupName](humanhandposeobservation/jointsgroupname.md)
  The joint group names available in the observation.


---

*[View on Apple Developer](https://developer.apple.com/documentation/vision/humanhandposeobservation/jointname)*