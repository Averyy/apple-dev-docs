# sparse_permute_rows_double(_:_:)
...
On successful return,  has been permuted and [`SPARSE_SUCCESS`](sparse_success.md) is returned.

#### Discussion

Permute the rows of the sparse matrix  based on the provided permutation array. For each row in , swap rows as:
...
- `A`: The sparse matrix,  .
- `perm`: The permutation array.  Holds number of rows in   values indicating the permutation of the matrix.  The indices in   are expected to be 0 based  (first element of pointer is  ). The indices in perm are expected to be within bounds of the matrix.  Undefined behavior if not met.
...
*[View on Apple Developer](https://developer.apple.com/documentation/accelerate/sparse_permute_rows_double(_:_:))*