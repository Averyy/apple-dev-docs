# sparse_matrix_vector_product_dense_double(_:_:_:_:_:_:_:)
...
Multiplies the dense vector  by the sparse matrix  and adds the result to the dense vector  (, where  is either  or the transpose of ).If the desired operation is , then an efficient option is to create the `y` buffer of zeros and then perform the operation with the zero filled `y`.
...
- `transa`: Specifies whether to perform the operation with A or the transpose of A. Must be one of   or  .
- `alpha`: Scalar multiplier of  .
- `A`: The sparse matrix,  .
- `x`: Pointer to the dense vector  . The dimension must be the number of columns of the matrix   when   is no transpose or the number of rows of the matrix   when   is transpose.  The behavior of this function is undefined if this is not met.  Negative strides are supported.  Note, unlike dense BLAS routines, the pointer points to the last element when stride is negative.
- `incx`: Increment between valid values in the dense vector x. Negative strides are supported.
- `y`: Pointer to the dense vector  . The dimension must be the number of rows of the matrix   when   is no transpose or the number of columns of the matrix   when   is transpose.  The behavior of this function is undefined if this is not met.  Negative strides are supported.  Note, unlike dense BLAS routines, the pointer points to the last element when stride is negative.
...
*[View on Apple Developer](https://developer.apple.com/documentation/accelerate/sparse_matrix_vector_product_dense_double(_:_:_:_:_:_:_:))*