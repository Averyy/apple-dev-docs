...
```swift
extern void vDSP_fft2d_zriptD(FFTSetupD __Setup, const DSPDoubleSplitComplex * __C, vDSP_Stride __IC0, vDSP_Stride __IC1, const DSPDoubleSplitComplex * __Buffer, vDSP_Length __Log2N0, vDSP_Length __Log2N1, FFTDirection __flag);
```
...
- `__Buffer`: A temporary vector that the operation uses for storing interim results. The real and imaginary parts of the buffer must both contain  max(Log2N0, Log2N1)  elements. For best performance, the buffer addresses must be 16-byte aligned or better.
...
- `__flag`: A flag that specifies the transform direction. Pass   to transform from the spatial domain to the frequency domain. Pass   to transform from the frequency domain to the spatial domain.

## See Also

- [vDSP_fft2d_zript](vdsp_fft2d_zript.md)
  Computes a 2D forward or inverse in-place, single-precision real FFT using a temporary buffer.


---

*[View on Apple Developer](https://developer.apple.com/documentation/accelerate/vdsp_fft2d_zriptd)*