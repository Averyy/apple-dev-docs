...
The following sample implementation of [`UserStartController`](iouserscsiparallelinterfacecontroller/userstartcontroller.md) sets its local power state to [`kIOServicePowerCapabilityOn`](https://developer.apple.com/documentation/driverkit/3325571-service_power_capabilities/kioservicepowercapabilityon), then calls a hypothetical `EnableReplyInterrupt()` convenience function. Next, it enables the interrupt dispatch source, and returns [`kIOReturnSuccess`](https://developer.apple.com/documentation/DriverKit/kIOReturnSuccess). Signaling success like this indicates to the kernel that the dext is now up and ready to serve I/O.
