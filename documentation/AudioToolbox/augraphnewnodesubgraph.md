...
This function creates a node that represents a subgraph. The subgraph can be retrieved with the [`AUGraphGetNodeInfoSubGraph(_:_:_:)`](augraphgetnodeinfosubgraph(_:_:_:).md) function. The subgraph is owned by the parent graph and will be disposed when the parent graph is disposed or when the node is removed from the parent graph.
...
*[View on Apple Developer](https://developer.apple.com/documentation/audiotoolbox/augraphnewnodesubgraph(_:_:))*