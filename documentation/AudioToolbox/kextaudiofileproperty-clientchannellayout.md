...
This property’s value specifies the channel layout of the buffers in an `AudioBufferList` list that you pass to the [`ExtAudioFileRead(_:_:_:)`](extaudiofileread(_:_:_:).md) and [`ExtAudioFileWrite(_:_:_:)`](extaudiofilewrite(_:_:_:).md) functions. This layout may be different from the file’s channel layout, in which case the extended audio file object’s underlying audio converter performs remapping. This property must be set after setting the `kExtAudioFileProperty_ClientDataFormat` property, and the number of channels in the two layouts must match.
...
*[View on Apple Developer](https://developer.apple.com/documentation/audiotoolbox/kextaudiofileproperty_clientchannellayout)*