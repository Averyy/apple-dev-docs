...
This property can be used to modify the  [`kAudioSessionCategory_RecordAudio`](kaudiosessioncategory_recordaudio.md) or [`kAudioSessionCategory_PlayAndRecord`](kaudiosessioncategory_playandrecord.md) categories. Attempting to set this property to `TRUE` will fail for all other categories.

This property affects the [`kAudioSessionCategory_PlayAndRecord`](kaudiosessioncategory_playandrecord.md) category as follows: If the audio input to the device is coming from a Bluetooth headset, setting this property to `TRUE` results in audio output also going to the Bluetooth headset.
...
*[View on Apple Developer](https://developer.apple.com/documentation/audiotoolbox/kaudiosessionproperty_overridecategoryenablebluetoothinput)*