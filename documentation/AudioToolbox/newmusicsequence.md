...
To direct the output of a music sequence to an audio processing graph, use the [`MusicSequenceSetAUGraph(_:_:)`](musicsequencesetaugraph(_:_:).md) function. To direct the output instead to a MIDI endpoint, use the [`MusicSequenceSetMIDIEndpoint(_:_:)`](musicsequencesetmidiendpoint(_:_:).md) function.

To direct the output of a specific music track, use the [`MusicTrackSetDestNode(_:_:)`](musictracksetdestnode(_:_:).md) and [`MusicTrackSetDestMIDIEndpoint(_:_:)`](musictracksetdestmidiendpoint(_:_:).md) functions, described in `MusicTrack`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/audiotoolbox/newmusicsequence(_:))*