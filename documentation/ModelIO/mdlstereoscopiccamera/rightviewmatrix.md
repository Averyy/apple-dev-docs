...
Model I/O  automatically generates this matrix from the camera’s position and orientation, additionally using the [`interPupillaryDistance`](mdlstereoscopiccamera/interpupillarydistance.md) and [`rightVergence`](mdlstereoscopiccamera/rightvergence.md) properties to account for the offset and angle of a stereoscopic camera’s left viewpoint.

A renderer uses this matrix, along with the [`rightProjectionMatrix`](mdlstereoscopiccamera/rightprojectionmatrix.md) property and model matrices derived from the camera’s position and orientation (the [`transform`](mdlobject/transform.md) property) and the content to be rendered, to transform vertex data to the renderer’s 2D screen space at render time.
...
*[View on Apple Developer](https://developer.apple.com/documentation/modelio/mdlstereoscopiccamera/rightviewmatrix)*