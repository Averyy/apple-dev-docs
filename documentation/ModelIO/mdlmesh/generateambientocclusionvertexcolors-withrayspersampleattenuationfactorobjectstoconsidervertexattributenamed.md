...
The `raysPerSample` parameter controls the fidelity and performance of the AO calculation process and its output. To control quality without specifying raytracing options, use the [`generateAmbientOcclusionVertexColors(withQuality:attenuationFactor:objectsToConsider:vertexAttributeNamed:)`](mdlmesh/generateambientocclusionvertexcolors(withquality:attenuationfactor:objectstoconsider:vertexattributenamed:).md) method.
...
*[View on Apple Developer](https://developer.apple.com/documentation/modelio/mdlmesh/generateambientocclusionvertexcolors(withrayspersample:attenuationfactor:objectstoconsider:vertexattributenamed:))*