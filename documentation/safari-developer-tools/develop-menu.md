...
If you haven’t already enabled features for web developers in Safari, follow the instruction for [`Enabling features for web developers`](enabling-developer-features.md). Once enabled, the  menu will be shown in the menu bar for Safari.
...
Use feature detection to enable or disable functionality instead of relying on the name or version of the browser in the User Agent string. [`Learn how to detect available features on MDN…`](https://developer.apple.comhttps://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing/Cross_browser_testing/Feature_detection)
