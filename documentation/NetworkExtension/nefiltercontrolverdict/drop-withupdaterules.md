...
When the Filter Control Provider passes this verdict to the completion handler passed to its [`handleNewFlow(_:completionHandler:)`](nefiltercontrolprovider/handlenewflow(_:completionhandler:).md) method, the system will drop all of the flow’s data. In addition, if the updateRules parameter is YES the system will call the Filter Data Provider’s [`handleRulesChanged()`](nefilterdataprovider/handleruleschanged().md) method.

## Parameters

- `updateRules`: A Boolean indicating whether or not the Filter Control Provider updated the rules.
...
*[View on Apple Developer](https://developer.apple.com/documentation/networkextension/nefiltercontrolverdict/drop(withupdaterules:))*