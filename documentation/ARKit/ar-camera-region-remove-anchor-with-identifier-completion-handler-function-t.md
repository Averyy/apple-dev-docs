# ar_camera_region_remove_anchor_with_identifier_completion_handler_function_t
...
```swift
typedef void (*)(void *, unsigned char *, _Bool, NSObject<OS_ar_error> *) ar_camera_region_remove_anchor_with_identifier_completion_handler_function_t;
```
...
- `successful`:  if the camera region anchor was successfully removed,   otherwise.
- `error`: The error that occurred, if any. The returned error supports ARC. In non-ARC files, use   to release the error.
...
*[View on Apple Developer](https://developer.apple.com/documentation/arkit/ar_camera_region_remove_anchor_with_identifier_completion_handler_function_t)*