...
```swift
errno_t VNOP_GETXATTR(vnode_t vp, const char *name, uio_t uio, size_t *size, int options, vfs_context_t ctx);
```
...
- `vp`: The vnode to get extended attributes for.
- `name`: Which property to extract.
- `uio`: Destination information for attribute value.
- `size`: Should be set to the amount of data written.
- `options`: XATTR_NOSECURITY: bypass security-checking.
- `ctx`: Context to authenticate for getxattr request.
...
*[View on Apple Developer](https://developer.apple.com/documentation/kernel/1586289-vnop_getxattr)*