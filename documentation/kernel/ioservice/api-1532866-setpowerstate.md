...
A power managed driver must override [`setPowerState`](ioservice/1532866-setpowerstate.md) to take part in system power management. After a driver registers with power management, the system uses [`setPowerState`](ioservice/1532866-setpowerstate.md) to power the device off and on for system sleep and wake. Calls to [`PMinit`](ioservice/1810486-pminit.md) and [`registerPowerDriver`](ioservice/1810706-registerpowerdriver.md) enable power management to change a device’s power state using [`setPowerState`](ioservice/1532866-setpowerstate.md). The system calls [`setPowerState`](ioservice/1532866-setpowerstate.md) in a clean and separate thread context.
...
*[View on Apple Developer](https://developer.apple.com/documentation/kernel/ioservice/1532866-setpowerstate)*