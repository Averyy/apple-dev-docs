...
After power management instructs a driver to change its state by calling [`setPowerState`](ioservice/1532866-setpowerstate.md), that driver must acknowledge the change when its device has completed its transition. The acknowledgement may be immediate, with a return code from [`setPowerState`](ioservice/1532866-setpowerstate.md), or delayed, with this call to [`acknowledgeSetPowerState`](ioservice/1532596-acknowledgesetpowerstate.md). Any driver that doesn’t return [`kIOPMAckImplied`](1645020-anonymous/kiopmackimplied.md) from its [`setPowerState`](ioservice/1532866-setpowerstate.md) implementation must later call [`acknowledgeSetPowerState`](ioservice/1532596-acknowledgesetpowerstate.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/kernel/ioservice/1532596-acknowledgesetpowerstate)*