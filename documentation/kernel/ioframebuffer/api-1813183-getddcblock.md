...
- `blockNumber`: Block number, ranging from one to the number of blocks return by the display.
- `blockType`: kIODDCBlockTypeEDID will be passed.
- `options`: No options are currently defined.
- `data`: Caller allocated buffer to receive the blocks data.
- `length`: In/out parameter - callers allocated buffer size, driver returns actual size.
...
*[View on Apple Developer](https://developer.apple.com/documentation/kernel/ioframebuffer/1813183-getddcblock)*