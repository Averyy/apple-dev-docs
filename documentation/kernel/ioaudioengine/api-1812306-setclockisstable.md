...
```swift
virtual void setClockIsStable(
 boolclockIsStable);
```

## Parameters

- `clockIsStable`: TRUE tells CoreAudio to use an agressive PLL to quickly lock to the engine's sample rate while FALSE tells CoreAudio to adjust more slowly to perceived sample rate changes that might just be the result of an unstable clock.
...
*[View on Apple Developer](https://developer.apple.com/documentation/kernel/ioaudioengine/1812306-setclockisstable)*