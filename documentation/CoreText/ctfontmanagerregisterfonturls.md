...
- `enabled`: A Boolean value that indicates whether the font derived from the URL should be enabled for font descriptor matching and discoverable through [`CTFontManagerRequestFonts(_:_:)`](ctfontmanagerrequestfonts(_:_:).md).
- `registrationHandler`: A block called as errors arise or upon completion. The block’s `errors` parameter contains an array of [`CFError`](https://developer.apple.com/documentation/corefoundation/cferror) references; an empty array indicates no errors. Each error reference contains a [`CFArray`](https://developer.apple.com/documentation/corefoundation/cfarray) of font descriptors corresponding to [`kCTFontManagerErrorFontURLsKey`](kctfontmanagererrorfonturlskey.md). These URLs represent the font files causing the error and failing to register successfully. This block may be called multiple times during the registration process. The `done` parameter becomes [`true`](https://developer.apple.com/documentation/swift/true) when the registration process completes. Return [`false`](https://developer.apple.com/documentation/swift/false) from the block to stop the registration operation, like after receiving an error.
