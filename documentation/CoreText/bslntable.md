# BslnTable

**Framework**: Core Text  
**Kind**: struct

**Availability**:
- iOS ?+
- iPadOS ?+
- Mac Catalyst ?+
- macOS ?+
- tvOS ?+
- visionOS ?+
- watchOS ?+

## Declaration

```swift
struct BslnTable
```

## Topics

### Initializers
- [init()](bslntable/init.md)
- [init(version: Fixed, format: BslnTableFormat, defaultBaseline: UInt16, parts: BslnFormatUnion)](bslntable/init(version:format:defaultbaseline:parts:).md)
### Instance Properties
- [var defaultBaseline: UInt16](bslntable/defaultbaseline.md)
- [var format: BslnTableFormat](bslntable/format.md)
- [var parts: BslnFormatUnion](bslntable/parts.md)
- [var version: Fixed](bslntable/version.md)

## Relationships

### Conforms To
- [Sendable](../swift/sendable.md)

## See Also

- [struct AnchorPoint](anchorpoint.md)
- [struct AnchorPointTable](anchorpointtable.md)
- [struct AnkrTable](ankrtable.md)
- [struct BslnFormat0Part](bslnformat0part.md)
- [struct BslnFormat1Part](bslnformat1part.md)
- [struct BslnFormat2Part](bslnformat2part.md)
- [struct BslnFormat3Part](bslnformat3part.md)
- [struct BslnFormatUnion](bslnformatunion.md)
- [struct FontVariation](fontvariation.md)
- [struct JustDirectionTable](justdirectiontable.md)
- [struct JustPCAction](justpcaction.md)
- [struct JustPCActionSubrecord](justpcactionsubrecord.md)
- [struct JustPCConditionalAddAction](justpcconditionaladdaction.md)
- [struct JustPCDecompositionAction](justpcdecompositionaction.md)
- [struct JustPCDuctilityAction](justpcductilityaction.md)


---

*[View on Apple Developer](https://developer.apple.com/documentation/coretext/bslntable)*