# JustPCConditionalAddAction

**Framework**: Core Text  
**Kind**: struct

**Availability**:
- iOS ?+
- iPadOS ?+
- Mac Catalyst ?+
- macOS ?+
- tvOS ?+
- visionOS ?+
- watchOS ?+

## Declaration

```swift
struct JustPCConditionalAddAction
```

## Topics

### Initializers
- [init()](justpcconditionaladdaction/init.md)
- [init(substThreshold: Fixed, addGlyph: UInt16, substGlyph: UInt16)](justpcconditionaladdaction/init(substthreshold:addglyph:substglyph:).md)
### Instance Properties
- [var addGlyph: UInt16](justpcconditionaladdaction/addglyph.md)
- [var substGlyph: UInt16](justpcconditionaladdaction/substglyph.md)
- [var substThreshold: Fixed](justpcconditionaladdaction/substthreshold.md)

## Relationships

### Conforms To
- [BitwiseCopyable](../swift/bitwisecopyable.md)
- [Sendable](../swift/sendable.md)

## See Also

- [struct AnchorPoint](anchorpoint.md)
- [struct AnchorPointTable](anchorpointtable.md)
- [struct AnkrTable](ankrtable.md)
- [struct BslnFormat0Part](bslnformat0part.md)
- [struct BslnFormat1Part](bslnformat1part.md)
- [struct BslnFormat2Part](bslnformat2part.md)
- [struct BslnFormat3Part](bslnformat3part.md)
- [struct BslnFormatUnion](bslnformatunion.md)
- [struct BslnTable](bslntable.md)
- [struct FontVariation](fontvariation.md)
- [struct JustDirectionTable](justdirectiontable.md)
- [struct JustPCAction](justpcaction.md)
- [struct JustPCActionSubrecord](justpcactionsubrecord.md)
- [struct JustPCDecompositionAction](justpcdecompositionaction.md)
- [struct JustPCDuctilityAction](justpcductilityaction.md)


---

*[View on Apple Developer](https://developer.apple.com/documentation/coretext/justpcconditionaladdaction)*