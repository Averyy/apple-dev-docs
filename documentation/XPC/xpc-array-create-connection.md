...
A new connection created from the value at the specified index. You are responsible for calling [`xpc_release`](xpc_release.md) on the returned connection. `NULL` if the value at the specified index is not an endpoint containing a connection. Each call to this method for the same index in the same array will yield a different connection. See [`xpc_connection_create_from_endpoint(_:)`](xpc_connection_create_from_endpoint(_:).md) for discussion as to the responsibilities when dealing with the returned connection.
...
*[View on Apple Developer](https://developer.apple.com/documentation/xpc/xpc_array_create_connection(_:_:))*