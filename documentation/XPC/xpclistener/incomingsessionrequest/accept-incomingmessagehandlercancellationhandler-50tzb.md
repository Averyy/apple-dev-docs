...
```swift
@preconcurrency
func accept<Message>(incomingMessageHandler: @escaping (Message) -> (any Encodable)?, cancellationHandler: ((XPCRichError) -> Void)? = nil) -> (XPCListener.IncomingSessionRequest.Decision, XPCSession) where Message : Decodable
```
