...
An activity in this state has just completed a check-in with the system after providing [`XPC_ACTIVITY_CHECK_IN`](xpc_activity_check_in.md) as the criteria dictionary to [`xpc_activity_register(_:_:_:)`](xpc_activity_register(_:_:_:).md). The state gives the app an opportunity to inspect and modify the activity’s criteria.
...
*[View on Apple Developer](https://developer.apple.com/documentation/xpc/xpc_activity_state_check_in)*