...
This property is an ordered pair that describes input values along the *x* axis for which the [`evaluate(x:)`](phaseenvelope/evaluate(x:).md) function can produce a non-zero output. The first number in the pair is the minimum input value, and the second number in the pair is the maximum input value.
