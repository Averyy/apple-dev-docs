...
For apps that can’t produce a minimal edit layer, export the complete scene and call [`reload()`](https://developer.apple.com/documentation/USDKit/USDStage/reload()) or [`reload()`](https://developer.apple.com/documentation/USDKit/USDLayer/reload()) on the USDKit side. USDKit diffs the old and new stage internally; [`USDPreviewSession`](usdpreviewsession.md) syncs only the deltas. This approach requires less bridging code but exports the full scene on every change, which can be slow for large scenes:
