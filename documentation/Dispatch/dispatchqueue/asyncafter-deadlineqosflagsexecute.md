...
```swift
@preconcurrency
func asyncAfter(deadline: DispatchTime, qos: DispatchQoS = .unspecified, flags: DispatchWorkItemFlags = [], execute work: @escaping @Sendable () -> Void)
```

## Parameters

- `deadline`: The time at which to schedule the block for execution. Specifying the current time is less efficient than calling the [`async(execute:)`](dispatchqueue/async(execute:).md) method directly. Do not specify the value in [`distantFuture`](dispatchtime/distantfuture.md); doing so is undefined.
...
*[View on Apple Developer](https://developer.apple.com/documentation/dispatch/dispatchqueue/asyncafter(deadline:qos:flags:execute:))*