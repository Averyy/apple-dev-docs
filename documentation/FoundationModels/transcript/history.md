...
When reading, if the first entry in the transcript is an [`Transcript.Entry.instructions(_:)`](transcript/entry/instructions(_:).md) entry, it is excluded from the returned slice. All other entries, including any subsequent instructions entries, are included.
