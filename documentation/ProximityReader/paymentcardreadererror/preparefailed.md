...
This error occurs when a call to the [`prepare(using:)`](paymentcardreader/prepare(using:).md) method fails. Check the description for more details and try to prepare the reader again. If the error persists, contact support.
...
*[View on Apple Developer](https://developer.apple.com/documentation/proximityreader/paymentcardreadererror/preparefailed(_:))*