...
If you already have data in Metal resources, use a [`MTLBlitCommandEncoder`](mtlblitcommandencoder.md) to copy data to and from different mipmaps in a texture.

To copy all matching data between two textures, encode a command using the [`copy(from:to:)`](mtlblitcommandencoder/copy(from:to:).md): method. The two textures must have the same pixel format and type. Metal copies all matching mipmap sizes to the destination texture.
