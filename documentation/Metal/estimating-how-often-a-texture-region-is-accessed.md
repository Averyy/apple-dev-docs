...
To get the current access counts for a sparse texture, create a blit command encoder and encode commands to copy the GPU’s internal counters to a [`MTLBuffer`](mtlbuffer.md) object. The following example code takes a tile region in the texture’s top-level mipmap, creates a [`MTLBuffer`](mtlbuffer.md) object large enough to accommodate all of the region’s tiles, and encodes a command to copy the counters.
