...
This descriptor can represent arguments within flat structures only. It can represent arrays of allowed argument buffer data types, but it cannot represent arguments within nested structures. Argument buffers with simple, flat structures can be represented by an array of [`MTLArgumentDescriptor`](mtlargumentdescriptor.md) objects. You can then use this array to create a [`MTLArgumentEncoder`](mtlargumentencoder.md) object by calling the [`makeArgumentEncoder(arguments:)`](mtldevice/makeargumentencoder(arguments:).md) method.Argument buffers with complex, nested structures must define their structure in Metal shading language code, which can then be directly assigned to a specific buffer index of a function. You can then use this buffer index to call the [`makeArgumentEncoder(bufferIndex:)`](mtlfunction/makeargumentencoder(bufferindex:).md) method and create a [`MTLArgumentEncoder`](mtlargumentencoder.md) object.
