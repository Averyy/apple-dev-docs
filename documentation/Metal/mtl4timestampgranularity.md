...
Pass these values to [`writeTimestamp(granularity:counterHeap:index:)`](mtl4computecommandencoder/writetimestamp(granularity:counterheap:index:).md) and [`writeTimestamp(granularity:after:counterHeap:index:)`](mtl4rendercommandencoder/writetimestamp(granularity:after:counterheap:index:).md) to control the desired accurracy of the counter sampling operation.

## Topics

### Enumeration Cases
- [MTL4TimestampGranularity.precise](mtl4timestampgranularity/precise.md)
  A timestamp as precise as possible.
- [MTL4TimestampGranularity.relaxed](mtl4timestampgranularity/relaxed.md)
  A minimally-invasive timestamp which may be less precise.
### Initializers
- [init?(rawValue: Int)](mtl4timestampgranularity/init(rawvalue:).md)
...
*[View on Apple Developer](https://developer.apple.com/documentation/metal/mtl4timestampgranularity)*