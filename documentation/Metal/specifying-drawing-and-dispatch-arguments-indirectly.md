...
An indirect command obtains its arguments from data stored in a [`MTLBuffer`](mtlbuffer.md) object, with a specific data layout for each kind of drawing or dispatch command. The supported layouts are defined by the following structures:
