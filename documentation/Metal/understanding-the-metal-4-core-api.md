...
- You submit a command buffer to any [`MTL4CommandQueue`](mtl4commandqueue.md) instance that belongs to the same device by calling one of its methods, such as [`commit:count:`](mtl4commandqueue/commit:count:.md), unlike [`MTLCommandBuffer`](mtlcommandbuffer.md) which has its own [`commit()`](mtlcommandbuffer/commit().md) method.
...
Use [`MTL4MachineLearningCommandEncoder`](mtl4machinelearningcommandencoder.md) to encode inference commands that apply [`Core ML`](https://developer.apple.com/documentation/coreml) models into a command buffer, alongside your app’s rendering and computation workloads. For more information, see [`Machine learning passes`](machine-learning-passes.md).
