...
Present when Headroom-Adaptive tone mapping is encoded. Contains `kColorSyncBaselineHeadroomStops` and either `kColorSyncHeadroomAdaptiveGainCurveInfo` or, when set to `kCFBooleanFalse`, indicates the Reference-White Tone Mapping Operator (RWTMO) method.
