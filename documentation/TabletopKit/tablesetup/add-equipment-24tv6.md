# add(equipment:)

**Framework**: TabletopKit  
**Kind**: method

**Availability**:
- visionOS 2.0+

## Declaration

```swift
mutating func add(equipment: some EntityEquipment)
```

## See Also

- [func add(equipment: some Equipment)](tablesetup/add(equipment:)-29pef.md)
- [func add(equipment: some Sequence)](tablesetup/add(equipment:)-4k6m6.md)
- [func add(equipment: some Sequence)](tablesetup/add(equipment:)-9syh2.md)


---

*[View on Apple Developer](https://developer.apple.com/documentation/tabletopkit/tablesetup/add(equipment:)-24tv6)*