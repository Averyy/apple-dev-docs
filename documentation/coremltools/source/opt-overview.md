...
- [.md](../_sources/source/opt-overview.md)
- **
...
Visit [What’s New](opt-whats-new.md) to see what is available on different
OS versions. Go over the [example overview section](opt-overview-examples.md)
...
[Optimization Workflow](opt-workflow.md).
...
Read more in the [palettization section](opt-palettization.md).
...
Read more in the [quantization section](opt-quantization.md).
- **Pruning**: refers to zeroing out values that are close to zero. Core ML supports sparse representations for weights.
Read more in the [pruning section](opt-pruning.md).

These techniques can be combined as well. For example, a joint sparse and palettized model or a joint sparse and quantized weights model can result in further compression and runtime performance gains. See [Combining Compression Types](opt-joint-compression.md)
