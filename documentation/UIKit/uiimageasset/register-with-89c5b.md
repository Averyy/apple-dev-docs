...
> ❗ **Important**:  The trait collection in `configuration` must always contain an explicit value in its [`displayScale`](uitraitcollection/displayscale.md) property. You may experience unexpected results from [`image(with:)`](uiimageasset/image(with:)-8jdwv.md) if the trait collection doesn’t explicitly define the desired scale.

 The trait collection in `configuration` must always contain an explicit value in its [`displayScale`](uitraitcollection/displayscale.md) property. You may experience unexpected results from [`image(with:)`](uiimageasset/image(with:)-8jdwv.md) if the trait collection doesn’t explicitly define the desired scale.
...
*[View on Apple Developer](https://developer.apple.com/documentation/uikit/uiimageasset/register(_:with:)-89c5b)*