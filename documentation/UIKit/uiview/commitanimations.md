...
Animations run in a separate thread to avoid blocking the application. In this way, multiple animations can be piled on top of one another. See [`setAnimationBeginsFromCurrentState(_:)`](uiview/setanimationbeginsfromcurrentstate(_:).md) for how to start animations while others are in progress.
...
*[View on Apple Developer](https://developer.apple.com/documentation/uikit/uiview/commitanimations())*