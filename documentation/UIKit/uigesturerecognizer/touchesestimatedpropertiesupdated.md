...
UIKit calls this method to report updates to properties that were previously declared to be estimates through [`touchesBegan(_:with:)`](uigesturerecognizer/touchesbegan(_:with:).md),  [`touchesMoved(_:with:)`](uigesturerecognizer/touchesmoved(_:with:).md) or [`touchesEnded(_:with:)`](uiresponder/touchesended(_:with:).md), and where declared to expect updates by having at least one property set in [`estimatedPropertiesExpectingUpdates`](uitouch/estimatedpropertiesexpectingupdates.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/uikit/uigesturerecognizer/touchesestimatedpropertiesupdated(_:))*