# HKWorkoutActivityType

**Framework**: HealthKit  
**Kind**: enum

The type of activity performed during a workout.

**Availability**:
- iOS 8.0+
- iPadOS 8.0+
- Mac Catalyst 13.0+
- macOS 13.0+
- visionOS 1.0+
- watchOS 2.0+

## Declaration

```swift
enum HKWorkoutActivityType
```

## Mentions

- [Dividing a HealthKit workout into activities](dividing-a-healthkit-workout-into-activities.md)

## Topics

### Individual sports
- [HKWorkoutActivityType.archery](hkworkoutactivitytype/archery.md)
  The constant for shooting archery.
- [HKWorkoutActivityType.bowling](hkworkoutactivitytype/bowling.md)
  The constant for bowling.
- [HKWorkoutActivityType.fencing](hkworkoutactivitytype/fencing.md)
  The constant for fencing.
- [HKWorkoutActivityType.gymnastics](hkworkoutactivitytype/gymnastics.md)
  Performing gymnastics.
- [HKWorkoutActivityType.trackAndField](hkworkoutactivitytype/trackandfield.md)
  Participating in track and field events, including shot put, javelin, pole vaulting, and related sports.
### Team sports
- [HKWorkoutActivityType.americanFootball](hkworkoutactivitytype/americanfootball.md)
  The constant for playing American football.
- [HKWorkoutActivityType.australianFootball](hkworkoutactivitytype/australianfootball.md)
  The constant for playing Australian football.
- [HKWorkoutActivityType.baseball](hkworkoutactivitytype/baseball.md)
  The constant for playing baseball.
- [HKWorkoutActivityType.basketball](hkworkoutactivitytype/basketball.md)
  The constant for playing basketball.
- [HKWorkoutActivityType.cricket](hkworkoutactivitytype/cricket.md)
  The constant for playing cricket.
- [HKWorkoutActivityType.discSports](hkworkoutactivitytype/discsports.md)
  The constant for playing disc sports such as Ultimate and Disc Golf.
- [HKWorkoutActivityType.handball](hkworkoutactivitytype/handball.md)
  The constant for playing handball.
- [HKWorkoutActivityType.hockey](hkworkoutactivitytype/hockey.md)
  The constant for playing hockey, including ice hockey, field hockey, and related sports.
- [HKWorkoutActivityType.lacrosse](hkworkoutactivitytype/lacrosse.md)
  The constant for playing lacrosse.
- [HKWorkoutActivityType.rugby](hkworkoutactivitytype/rugby.md)
  The constant for playing rugby.
- [HKWorkoutActivityType.soccer](hkworkoutactivitytype/soccer.md)
  The constant for playing soccer.
- [HKWorkoutActivityType.softball](hkworkoutactivitytype/softball.md)
  The constant for playing softball.
- [HKWorkoutActivityType.volleyball](hkworkoutactivitytype/volleyball.md)
  The constant for playing volleyball.
### Exercise and fitness
- [HKWorkoutActivityType.preparationAndRecovery](hkworkoutactivitytype/preparationandrecovery.md)
  The constant for warm-up and therapeutic activities like foam rolling and stretching.
- [HKWorkoutActivityType.flexibility](hkworkoutactivitytype/flexibility.md)
  The constant for a flexibility workout.
- [HKWorkoutActivityType.cooldown](hkworkoutactivitytype/cooldown.md)
  The constant for low intensity stretching and mobility exercises following a more vigorous workout.
- [HKWorkoutActivityType.walking](hkworkoutactivitytype/walking.md)
  The constant for walking.
- [HKWorkoutActivityType.running](hkworkoutactivitytype/running.md)
  The constant for running and jogging.
- [HKWorkoutActivityType.wheelchairWalkPace](hkworkoutactivitytype/wheelchairwalkpace.md)
  The constant for a wheelchair workout at walking pace.
- [HKWorkoutActivityType.wheelchairRunPace](hkworkoutactivitytype/wheelchairrunpace.md)
  The constant for wheelchair workout at running pace.
- [HKWorkoutActivityType.cycling](hkworkoutactivitytype/cycling.md)
  The constant for cycling.
- [HKWorkoutActivityType.handCycling](hkworkoutactivitytype/handcycling.md)
  The constant for hand cycling.
- [HKWorkoutActivityType.coreTraining](hkworkoutactivitytype/coretraining.md)
  The constant for core training.
- [HKWorkoutActivityType.elliptical](hkworkoutactivitytype/elliptical.md)
  The constant for workouts on an elliptical machine.
- [HKWorkoutActivityType.functionalStrengthTraining](hkworkoutactivitytype/functionalstrengthtraining.md)
  The constant for strength training, primarily with free weights and body weight.
- [HKWorkoutActivityType.traditionalStrengthTraining](hkworkoutactivitytype/traditionalstrengthtraining.md)
  The constant for strength training exercises primarily using machines or free weights.
- [HKWorkoutActivityType.crossTraining](hkworkoutactivitytype/crosstraining.md)
  The constant for exercise that includes any mixture of cardio, strength, and/or flexibility training.
- [HKWorkoutActivityType.mixedCardio](hkworkoutactivitytype/mixedcardio.md)
  The constant for workouts that mix a variety of cardio exercise machines or modalities.
- [HKWorkoutActivityType.highIntensityIntervalTraining](hkworkoutactivitytype/highintensityintervaltraining.md)
  The constant for high intensity interval training.
- [HKWorkoutActivityType.jumpRope](hkworkoutactivitytype/jumprope.md)
  The constant for jumping rope.
- [HKWorkoutActivityType.stairClimbing](hkworkoutactivitytype/stairclimbing.md)
  The constant for workouts using a stair climbing machine.
- [HKWorkoutActivityType.stairs](hkworkoutactivitytype/stairs.md)
  The constant for running, walking, or other drills using stairs (for example, in a stadium or inside a multilevel building).
- [HKWorkoutActivityType.stepTraining](hkworkoutactivitytype/steptraining.md)
  The constant for training using a step bench.
- [HKWorkoutActivityType.fitnessGaming](hkworkoutactivitytype/fitnessgaming.md)
  The constant for playing fitness-based video games.
### Studio activities
- [HKWorkoutActivityType.barre](hkworkoutactivitytype/barre.md)
  The constant for barre workout.
- [HKWorkoutActivityType.cardioDance](hkworkoutactivitytype/cardiodance.md)
  The constant for cardiovascular dance workouts.
- [HKWorkoutActivityType.socialDance](hkworkoutactivitytype/socialdance.md)
  The constant for dancing with a partner or partners, such as swing, salsa, or folk dances.
- [HKWorkoutActivityType.yoga](hkworkoutactivitytype/yoga.md)
  The constant for practicing yoga.
- [HKWorkoutActivityType.mindAndBody](hkworkoutactivitytype/mindandbody.md)
  The constant for performing activities like walking meditation, Gyrotonic exercise, and Qigong.
- [HKWorkoutActivityType.pilates](hkworkoutactivitytype/pilates.md)
  The constant for a pilates workout.
### Racket sports
- [HKWorkoutActivityType.badminton](hkworkoutactivitytype/badminton.md)
  The constant for playing badminton.
- [HKWorkoutActivityType.pickleball](hkworkoutactivitytype/pickleball.md)
  The constant for playing pickleball.
- [HKWorkoutActivityType.racquetball](hkworkoutactivitytype/racquetball.md)
  The constant for playing racquetball.
- [HKWorkoutActivityType.squash](hkworkoutactivitytype/squash.md)
  The constant for playing squash.
- [HKWorkoutActivityType.tableTennis](hkworkoutactivitytype/tabletennis.md)
  The constant for playing table tennis.
- [HKWorkoutActivityType.tennis](hkworkoutactivitytype/tennis.md)
  The constant for playing tennis.
### Outdoor activities
- [HKWorkoutActivityType.climbing](hkworkoutactivitytype/climbing.md)
  The constant for climbing.
- [HKWorkoutActivityType.equestrianSports](hkworkoutactivitytype/equestriansports.md)
  The constant for activities that involve riding a horse, including polo, horse racing, and horse riding.
- [HKWorkoutActivityType.fishing](hkworkoutactivitytype/fishing.md)
  The constant for fishing.
- [HKWorkoutActivityType.golf](hkworkoutactivitytype/golf.md)
  The constant for playing golf.
- [HKWorkoutActivityType.hiking](hkworkoutactivitytype/hiking.md)
  The constant for hiking.
- [HKWorkoutActivityType.hunting](hkworkoutactivitytype/hunting.md)
  The constant for hunting.
- [HKWorkoutActivityType.play](hkworkoutactivitytype/play.md)
  The constant for play-based activities like tag, dodgeball, hopscotch, tetherball, and playing on a jungle gym.
### Snow and ice sports
- [HKWorkoutActivityType.crossCountrySkiing](hkworkoutactivitytype/crosscountryskiing.md)
  The constant for cross country skiing.
- [HKWorkoutActivityType.curling](hkworkoutactivitytype/curling.md)
  The constant for curling.
- [HKWorkoutActivityType.downhillSkiing](hkworkoutactivitytype/downhillskiing.md)
  The constant for downhill skiing.
- [HKWorkoutActivityType.snowSports](hkworkoutactivitytype/snowsports.md)
  The constant for a variety of snow sports, including sledding, snowmobiling, or building a snowman.
- [HKWorkoutActivityType.snowboarding](hkworkoutactivitytype/snowboarding.md)
  The constant for snowboarding.
- [HKWorkoutActivityType.skatingSports](hkworkoutactivitytype/skatingsports.md)
  The constant for skating activities, including ice skating, speed skating, inline skating, and skateboarding.
### Water activities
- [HKWorkoutActivityType.paddleSports](hkworkoutactivitytype/paddlesports.md)
  The constant for canoeing, kayaking, paddling an outrigger, paddling a stand-up paddle board, and related sports.
- [HKWorkoutActivityType.rowing](hkworkoutactivitytype/rowing.md)
  The constant for rowing.
- [HKWorkoutActivityType.sailing](hkworkoutactivitytype/sailing.md)
  The constant for sailing.
- [HKWorkoutActivityType.surfingSports](hkworkoutactivitytype/surfingsports.md)
  The constant for a variety of surf sports, including surfing, kite surfing, and wind surfing.
- [HKWorkoutActivityType.swimming](hkworkoutactivitytype/swimming.md)
  The constant for swimming.
- [HKWorkoutActivityType.underwaterDiving](hkworkoutactivitytype/underwaterdiving.md)
  The constant for underwater diving.
- [HKWorkoutActivityType.waterFitness](hkworkoutactivitytype/waterfitness.md)
  The constant for aerobic exercise performed in shallow water.
- [HKWorkoutActivityType.waterPolo](hkworkoutactivitytype/waterpolo.md)
  The constant for playing water polo.
- [HKWorkoutActivityType.waterSports](hkworkoutactivitytype/watersports.md)
  The constant for a variety of water sports, including water skiing, wake boarding, and related activities.
### Martial arts
- [HKWorkoutActivityType.boxing](hkworkoutactivitytype/boxing.md)
  The constant for boxing.
- [HKWorkoutActivityType.kickboxing](hkworkoutactivitytype/kickboxing.md)
  The constant for kickboxing.
- [HKWorkoutActivityType.martialArts](hkworkoutactivitytype/martialarts.md)
  The constant for practicing martial arts.
- [HKWorkoutActivityType.taiChi](hkworkoutactivitytype/taichi.md)
  The constant for tai chi.
- [HKWorkoutActivityType.wrestling](hkworkoutactivitytype/wrestling.md)
  The constant for wrestling.
### Other activities
- [HKWorkoutActivityType.other](hkworkoutactivitytype/other.md)
  The constant for a workout that does not match any of the other workout activity types.
### Deprecated activity types
- [HKWorkoutActivityType.dance](hkworkoutactivitytype/dance.md)
  The constant for dancing.
- [HKWorkoutActivityType.danceInspiredTraining](hkworkoutactivitytype/danceinspiredtraining.md)
  The constant for workouts inspired by dance, including Pilates, Barre, and Feldenkrais.
- [HKWorkoutActivityType.mixedMetabolicCardioTraining](hkworkoutactivitytype/mixedmetaboliccardiotraining.md)
  The constant for performing any mix of cardio-focused exercises.
### Multisport activities
- [HKWorkoutActivityType.swimBikeRun](hkworkoutactivitytype/swimbikerun.md)
  The constant for multisport activities like triathlons.
- [HKWorkoutActivityType.transition](hkworkoutactivitytype/transition.md)
  A constant for the transition time between activities in a multisport workout.
### Initializers
- [init?(rawValue: UInt)](hkworkoutactivitytype/init(rawvalue:).md)

## Relationships

### Conforms To
- [BitwiseCopyable](../Swift/BitwiseCopyable.md)
- [Equatable](../Swift/Equatable.md)
- [Hashable](../Swift/Hashable.md)
- [RawRepresentable](../Swift/RawRepresentable.md)
- [Sendable](../Swift/Sendable.md)
- [SendableMetatype](../Swift/SendableMetatype.md)

## See Also

- [Adding samples to a workout](adding-samples-to-a-workout.md)
  Create associated samples that add details to a workout.
- [Accessing condensed workout samples](accessing-condensed-workout-samples.md)
  Read series data from condensed workouts.
- [Dividing a HealthKit workout into activities](dividing-a-healthkit-workout-into-activities.md)
  Partition multisport and interval workouts into activities that represent the different parts of the workout.
- [class HKWorkout](hkworkout.md)
  A workout sample that stores information about a single physical activity.
- [class HKWorkoutActivity](hkworkoutactivity.md)
  An object that describes an activity within a longer workout.
- [class HKWorkoutBuilder](hkworkoutbuilder.md)
  A builder object that incrementally constructs a workout.
- [class HKWorkoutType](hkworkouttype.md)
  A type that identifies samples that store information about a workout.
- [let HKWorkoutTypeIdentifier: String](hkworkouttypeidentifier.md)
  The workout type identifier.
- [enum HKWorkoutSessionType](hkworkoutsessiontype.md)
  The type of session.
- [class HKWorkoutEvent](hkworkoutevent.md)
  An object representing an important event during a workout.


---

*[View on Apple Developer](https://developer.apple.com/documentation/healthkit/hkworkoutactivitytype)*