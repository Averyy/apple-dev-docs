...
The user must wear their Apple Watch for at least one day before the system can generate the first [`vo2Max`](hkquantitytypeidentifier/vo2max.md) sample. Additionally, the system doesn’t generate a [`vo2Max`](hkquantitytypeidentifier/vo2max.md) sample on the user’s first workout. However, it can make estimates based on data collected outside a workout session.
