...
Use this convenience method to create a predicate that matches workouts with the specified average quantity. For more information on how HealthKit calculates statistics for [`HKWorkout`](hkworkout.md) objects, see [`statistics(for:)`](hkworkout/statistics(for:).md) .
...
*[View on Apple Developer](https://developer.apple.com/documentation/healthkit/hkquery/predicateforworkouts(operatortype:quantitytype:averagequantity:))*