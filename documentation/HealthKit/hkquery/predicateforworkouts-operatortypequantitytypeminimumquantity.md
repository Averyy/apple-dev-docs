...
Use this convenience method to create a predicate that matches workouts with the specified minimum quantity. For more information on how HealthKit calculates statistics for [`HKWorkoutActivity`](hkworkoutactivity.md) objects, see [`statistics(for:)`](hkworkoutactivity/statistics(for:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/healthkit/hkquery/predicateforworkouts(operatortype:quantitytype:minimumquantity:))*