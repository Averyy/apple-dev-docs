...
| `atomic` | A Boolean value indicating whether the entire operation fails when one or more operations fail. ![None](/images/com.apple.cloudkitjs/spacer.png) If `true`, the entire request fails if one operation fails. If `false`, some operations may succeed and others may fail. The default value is `false`. ![None](/images/com.apple.cloudkitjs/spacer.png) This property only applies to custom zones. |
