...
```swift
var ES_EVENT_TYPE_NOTIFY_LOOKUP: es_event_type_t { get }
```
...
value corresponds to the [`es_events_t`](es_events_t.md) union member [`lookup`](es_events_t/lookup.md), which has the type [`es_event_lookup_t`](es_event_lookup_t.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/endpointsecurity/es_event_type_notify_lookup)*