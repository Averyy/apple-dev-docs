...
> **Note**:  Because this method may not present an alert, don’t call [`requestReview(in:)`](appstore/requestreview(in:)-4r0y9.md) in response to a button tap or other user action.

 Because this method may not present an alert, don’t call [`requestReview(in:)`](appstore/requestreview(in:)-4r0y9.md) in response to a button tap or other user action.
...
- `controller`: The   that StoreKit uses to present the rating and review request interface.
...
*[View on Apple Developer](https://developer.apple.com/documentation/storekit/appstore/requestreview(in:)-4r0y9)*