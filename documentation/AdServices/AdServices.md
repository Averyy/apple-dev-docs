...
- In step 2, the AdServices framework generates a token. For more detail, see [`attributionToken()`](aaattribution/attributiontoken().md).
- In step 3, an MMP or developer uses the token in a RESTful API request to retrieve an attribution record from Apple’s attribution server. For more detail, see [`Attribution payload`](aaattribution/attributiontoken()#Attribution-payload.md).
- In step 4, the attribution record that returns has key-value pairs that correspond to your campaigns in the Apple Search Ads Campaign Management API. For more detail, see [`Attribution payload descriptions`](aaattribution/attributiontoken()#Attribution-payload-descriptions.md).
