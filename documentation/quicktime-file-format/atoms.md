...
> **Note**: An atom, as this documentation describes, is functionally identical to a *box*, as the ISO specifications describe for MPEG-4 and Motion JPEG-2000. An atom that includes version and flags fields is functionally identical to a *full box* as those specifications define.
...
![A container atom with an atom size and atom type, that contains another container atom with an atom size and atom type. The inner container atom contains a leaf atom, with an atom size, atom type, and atom data.](/images/com.apple.qtff/sample-atom@2x.png)
...
- **Extended size**: If the `size` field of an atom is set to 1, the `type` field is followed by a 64-bit `extended size` field, which contains the actual size of the atom as a 64-bit unsigned integer. This is used when the size of a media data atom exceeds 2^32 bytes.
...
*[View on Apple Developer](https://developer.apple.com/documentation/quicktime-file-format/atoms)*