...
When a customer initiates a refund request for a consumable In-App Purchase or auto-renewable subscription, the App Store sends a `CONSUMPTION_REQUEST` [`notificationType`](https://developer.apple.com/documentation/AppStoreServerNotifications/notificationType) to your server through your [`App Store Server Notifications V2`](https://developer.apple.com/documentation/AppStoreServerNotifications/App-Store-Server-Notifications-V2) endpoint. If the customer provided consent, respond by calling this API and sending the consumption data in the [`ConsumptionRequestV1`](consumptionrequestv1.md) to the App Store. If not, don’t respond to the `CONSUMPTION_REQUEST` notification.
...
> ❗ **Important**:  Don’t use the [`App Tracking Transparency`](https://developer.apple.com/documentation/AppTrackingTransparency) prompt to obtain consent for sharing data with Apple through the [`Send Consumption Information V1`](send-consumption-information-v1.md) API. Obtaining consent needed to use the [`Send Consumption Information V1`](send-consumption-information-v1.md) endpoint is unrelated to [`App Tracking Transparency`](https://developer.apple.com/documentation/AppTrackingTransparency). These two features are distinct and unrelated.
...
*[View on Apple Developer](https://developer.apple.com/documentation/appstoreserverapi/send-consumption-information-v1)*