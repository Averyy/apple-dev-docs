...
```javascript
GET https://api.storekit.itunes.apple.com/inApps/v1/history/{anyTransactionId}?productType=NON_CONSUMABLE&productType=AUTO_RENEWABLE
```
...
```javascript
GET https://api.storekit.itunes.apple.com/inApps/v1/history/{anyTransactionId}
```

For subsequent requests, include the `revision` token from the previous [`HistoryResponse`](historyresponse.md).

```javascript
GET https://api.storekit.itunes.apple.com/inApps/v1/history/{anyTransactionId}?revision={revision}
```

## Endpoint

`GET https://api.storekit-sandbox.itunes.apple.com/inApps/v1/history/{anyTransactionId}`
