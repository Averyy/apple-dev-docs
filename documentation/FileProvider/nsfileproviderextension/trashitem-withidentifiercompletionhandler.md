...
Be sure to record the item’s parent identifier, so that you can restore the item to its proper location in the [`untrashItem(withIdentifier:toParentItemIdentifier:completionHandler:)`](nsfileproviderextension/untrashitem(withidentifier:toparentitemidentifier:completionhandler:).md) method. You can store this identifier in the `trashedItem` object’s [`parentItemIdentifier`](nsfileprovideritemprotocol/parentitemidentifier.md) property.
...
*[View on Apple Developer](https://developer.apple.com/documentation/fileprovider/nsfileproviderextension/trashitem(withidentifier:completionhandler:))*