...
This convenience function calculates the minimum amount of space needed in the output histogram for the results. The buffer should be at least this length and longer if the `histogramOffset` value in the [`encode(to:sourceTexture:histogram:histogramOffset:)`](mpsimagehistogram/encode(to:sourcetexture:histogram:histogramoffset:).md) method is non-zero.
...
*[View on Apple Developer](https://developer.apple.com/documentation/metalperformanceshaders/mpsimagehistogram/histogramsize(forsourceformat:))*