...
When you disable the find operation, you also implicitly disable the replace operation. If you want to only disable replace, use [`replaceDisabled(_:)`](view/replacedisabled(_:).md) instead.

Using this modifer also prevents programmatic find and replace interface presentation using the [`findNavigator(isPresented:)`](view/findnavigator(ispresented:).md) method. Be sure to place the disabling modifier closer to the text editor for this to work:
...
*[View on Apple Developer](https://developer.apple.com/documentation/swiftui/view/finddisabled(_:))*