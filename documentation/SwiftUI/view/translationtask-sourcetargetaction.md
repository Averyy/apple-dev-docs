...
To perform new translations with the same `source` and `target` language, use [`translationTask(_:action:)`](view/translationtask(_:action:).md) and call [`invalidate()`](https://developer.apple.com/documentation/translation/translationsession/configuration/invalidate()).
...
- `action`: A closure that runs when the view first appears and when `source` or `target` change.  It provides a [`TranslationSession`](https://developer.apple.com/documentation/translation/translationsession) instance to perform one or multiple translations.
