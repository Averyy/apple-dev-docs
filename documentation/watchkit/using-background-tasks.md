...
- Use an app delegate and implement the delegate’s [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md) method. This method must respond to all the background tasks generated by the system.
...
To specify a particular identifier, pass a [`String`](https://developer.apple.com/documentation/Swift/String) as the `userInfo` parameter for [`scheduleBackgroundRefresh(withPreferredDate:userInfo:scheduledCompletion:)`](wkapplication/schedulebackgroundrefresh(withpreferreddate:userinfo:scheduledcompletion:).md).
...
2. After the task completes, the app calls the task’s [`setTaskCompletedWithSnapshot(_:)`](wkrefreshbackgroundtask/settaskcompletedwithsnapshot(_:).md) method. The system suspends the app again as soon as all background tasks finish. The system gives your app only a few seconds of background execution time. Complete the background task and call [`setTaskCompletedWithSnapshot(_:)`](wkrefreshbackgroundtask/settaskcompletedwithsnapshot(_:).md) as quickly as possible.

Because background tasks happen asynchronously, the system may call your [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md) method with multiple tasks. It can also call [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md) while you’re already handling other background tasks, or even while your app is running in the foreground. If watchOS calls your [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md) method while your app is in the foreground, you can ignore the tasks, call [`setTaskCompletedWithSnapshot(_:)`](wkrefreshbackgroundtask/settaskcompletedwithsnapshot(_:).md), and return.

If you implement your delegate’s [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md) method, you receive all the background tasks, both those that you schedule and those that the system schedules. Some of your app’s other behaviors also change:
...
For some background tasks, the system calls other delegate methods or completion handlers in addition to calling [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md). The call to your delegate’s [`handle(_:)`](wkapplicationdelegate/handle(_:)-4vdjo.md) method occurs before any other methods or handlers. To process these multistep procedures, first save the task. Then, after you finish processing all the delegate method calls, call the task’s [`setTaskCompletedWithSnapshot(_:)`](wkrefreshbackgroundtask/settaskcompletedwithsnapshot(_:).md) method.
...
2. The system then calls your delegate’s [`handle(_:completionHandler:)`](wkextensiondelegate/handle(_:completionhandler:).md) method, passing an intent and completion handler. In this method, your app processes the intent and calls its completion handler.
3. Finally, mark the task as complete by calling its [`setTaskCompletedWithSnapshot(_:)`](wkrefreshbackgroundtask/settaskcompletedwithsnapshot(_:).md) method.
