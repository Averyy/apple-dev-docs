# defaultUnit

**Framework**: App Intents  
**Kind**: property

**Availability**:
- iOS 17.0+
- iPadOS 17.0+
- Mac Catalyst 17.0+
- macOS 14.0+
- tvOS 17.0+
- visionOS 1.0+
- watchOS 10.0+

## Declaration

```swift
final var defaultUnit: IntentParameter<Value>.Angle? { get }
```

## See Also

- [var unit: IntentParameter<Value>.Angle?](intentparameter/unit-egg9.md)
- [IntentParameter.Angle](intentparameter/angle.md)
- [var supportsNegativeNumbers: Bool?](intentparameter/supportsnegativenumbers-35m3l.md)
- [var unitAdjustForLocale: Bool?](intentparameter/unitadjustforlocale-9lff6.md)


---

*[View on Apple Developer](https://developer.apple.com/documentation/appintents/intentparameter/defaultunit-9huqe)*