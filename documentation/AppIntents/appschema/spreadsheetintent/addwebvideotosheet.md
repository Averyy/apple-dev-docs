...
```swift
var addWebVideoToSheet: some AppSchemaIntent { get }
```

#### Discussion

To make your app’s actions available to Apple Intelligence, conform your [`AppIntent`](appintent.md) to a schema that describes your action to the system. If your app’s functionality aligns with the `spreadsheet` domain and one of your app’s actions matches the `addWebVideoToSheet` schema, you can generate the properties and protocol conformance the schema requires for your intent implementation with the `@AppIntent( .spreadsheet.addWebVideoToSheet)` Swift macro. To make your app work with Siri, see [`Apple Intelligence and Siri AI`](apple-intelligence-and-siri-ai.md).

The following example shows an intent that conforms to the `addWebVideoToSheet` schema:

```swift
@AppIntent(schema: .spreadsheet.addWebVideoToSheet)
struct AddWebVideoToSheetIntent {
    var url: URL
...
*[View on Apple Developer](https://developer.apple.com/documentation/appintents/appschema/spreadsheetintent/addwebvideotosheet)*