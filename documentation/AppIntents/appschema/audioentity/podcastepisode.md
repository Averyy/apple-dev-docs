...
```swift
var podcastEpisode: some AppSchemaEntity { get }
```

#### Discussion

To make your app’s content available to Apple Intelligence, conform your [`AppEntity`](appentity.md) to a schema that describes your content to the system. If your app’s functionality aligns with the `audio` domain and its content matches the `podcastEpisode` schema, you can generate the properties and protocol conformance the schema requires for your app entity implementation with the `@AppEntity( .audio.podcastEpisode)` Swift macro. To make your app work with Siri, see [`Apple Intelligence and Siri AI`](apple-intelligence-and-siri-ai.md).

The following example shows an app entity that conforms to the `podcastEpisode` schema:
...
struct PodcastEpisodeEntity {
    ...
    struct PodcastEpisodeEntityQuery: EntityQuery {
        func entities(for identifiers: [PodcastEpisodeEntity.ID]) async throws -> [PodcastEpisodeEntity] {
            <#code#>
        }
    }
}
...
*[View on Apple Developer](https://developer.apple.com/documentation/appintents/appschema/audioentity/podcastepisode)*