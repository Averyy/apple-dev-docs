...
```swift
@attached
(peer, names: prefixed(`$`), prefixed(`_`)) @attached(accessor, names: named(get), named(set)) macro DeferredProperty(indexingKey: PartialKeyPath<CSSearchableItemAttributeSet>)
```
...
*[View on Apple Developer](https://developer.apple.com/documentation/appintents/deferredproperty(indexingkey:))*