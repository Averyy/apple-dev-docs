...
The associated value must be an [`NSNumber`](https://developer.apple.com/documentation/foundation/nsnumber) object that specifies a `BOOL` value (`true` or `false`). The default is `true`. This option has no effect if the image used for initialization is not RAW.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coreimage/cirawfilteroption/enablechromaticnoisetracking)*