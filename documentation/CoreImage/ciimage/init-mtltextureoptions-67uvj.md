...
To render the image using Metal, use this image with a Metal-based [`CIContext`](cicontext.md) object created with the [`init(mtlDevice:)`](cicontext/init(mtldevice:)-swey.md) method, and call the [`render(_:to:commandBuffer:bounds:colorSpace:)`](cicontext/render(_:to:commandbuffer:bounds:colorspace:).md) method to create an output image in another Metal texture object.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coreimage/ciimage/init(mtltexture:options:)-67uvj)*