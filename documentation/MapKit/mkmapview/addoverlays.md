...
The map view adds the specified objects to the group of overlay objects in the [`MKOverlayLevel.aboveLabels`](mkoverlaylevel/abovelabels.md) level. Adding an overlay causes the map view to begin monitoring the area that the overlay represents. As soon as the bounding rectangle of the overlay intersects the visible portion of the map, the map view tries to draw the overlay, or adds the corresponding overlay view to the map. Implement the [`mapView(_:rendererFor:)`](mkmapviewdelegate/mapview(_:rendererfor:).md) method of the map view’s delegate object to provide the overlay view.
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkit/mkmapview/addoverlays(_:))*