...
This method adds the specified overlay to the end of the list of overlay objects at the given level. Adding an overlay also causes the map view to begin monitoring the area they represent. As soon as the bounding rectangle of the overlay intersects the visible portion of the map, the map view calls your delegate’s [`mapView(_:rendererFor:)`](mkmapviewdelegate/mapview(_:rendererfor:).md) method to get the renderer object to use when drawing the overlay.
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkit/mkmapview/addoverlay(_:level:))*