...
```swift
@MainActor
@preconcurrency init(position: Binding<MapCameraPosition>, bounds: MapCameraBounds? = nil, interactionModes: MapInteractionModes = .all, selection: Binding<MapFeature?>, scope: Namespace.ID? = nil) where Content == MapContentView<MapFeature, EmptyMapContent>
```
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkit/map/init(position:bounds:interactionmodes:selection:scope:))*