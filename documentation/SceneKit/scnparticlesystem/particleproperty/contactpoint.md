...
The contact point property only applies to collision handler blocks (see the [`handle(_:forProperties:handler:)`](scnparticlesystem/handle(_:forproperties:handler:).md) method and [`SCNParticleEvent.collision`](scnparticleevent/collision.md) constant). Its value is a three-component vector (an array of three `float` values).
...
*[View on Apple Developer](https://developer.apple.com/documentation/scenekit/scnparticlesystem/particleproperty/contactpoint)*