...
Reading this property is equivalent to reading the [`simdLocalUp`](scnnode/simdlocalup.md) class property and using the [`simdConvertVector(_:to:)`](scnnode/simdconvertvector(_:to:).md) or [`simdConvertVector(_:from:)`](scnnode/simdconvertvector(_:from:).md) method to convert that vector from the node’s local coordinate space to the scene’s world coordinate space.
...
*[View on Apple Developer](https://developer.apple.com/documentation/scenekit/scnnode/simdworldup)*