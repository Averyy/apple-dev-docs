...
Setting this option or the [`sharedLock`](filedescriptor/openoptions/sharedlock.md) option. obtains a lock with `flock(2)` semantics. If you’re creating a file using the [`create`](filedescriptor/openoptions/create.md) option, the request for the lock always succeeds except on file systems that don’t support locking.
...
*[View on Apple Developer](https://developer.apple.com/documentation/system/filedescriptor/openoptions/exclusivelock)*