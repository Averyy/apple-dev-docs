...
- `AlwaysOff` - The device isn’t allowed to enroll in any beta programs. The system removes the device from any beta programs, if already enrolled. Allowed enrollments: supervised
- `RequireProgram` (SoftwareUpdateSettingsBeta_RequireProgramObject): The device automatically enrolls in this beta program. This key must only be present if the `ProgramEnrollment` key is set to `AlwaysOn`. The `OfferPrograms` key must not be present if this key is present. Allowed enrollments: supervised
