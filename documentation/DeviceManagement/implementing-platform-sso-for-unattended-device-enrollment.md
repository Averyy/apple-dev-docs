...
> **Note**:  Declarative device management is the preferred solution. It requires less network traffic to monitor the app status because the device reports the status autonomously. With MDM commands, the device management service needs to continuously poll the device to check the app status.
