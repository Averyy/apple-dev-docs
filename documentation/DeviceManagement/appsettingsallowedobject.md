...
- `AlwaysAllowManagedApps` (boolean): If `true`, the device implicitly includes managed apps in the effective allow list when `AllowedBinaries` is present. Available: macOS 27+
Allowed scopes: system
...
- `DeniedBinaries` ([AppSettingsAllowed_BinaryIdentifierObject]): If present, the device doesn’t allow binaries that match the binary identifier properties to run. A binary only matches when all the binary identifiers match. Available: macOS 27+
Allowed scopes: system
