...
If the `TargetVersion` is an OS version that includes both a minor and patch version, the system installs that specific version, for example, `16.1.1`. If the minor version doesn’t include a patch version, the system installs the latest available patch version. For example, if the `TargetVersion` is `16.1` and a `.1` patch is available, the system installs `16.1.1`.
...
If the device isn’t running at the target date-time, the system enforces the update 1 hour after restarting, or when the device meets all required conditions, such as minimum battery level.
