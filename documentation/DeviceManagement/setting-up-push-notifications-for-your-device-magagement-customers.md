...
> **Note**:  Device management doesn’t support the token based push process.
...
> **Note**:  The private key for this CSR must remain within the instance of your device management service that the customer has access to. You use this private key to sign the device management push certificate. Be certain that the device management service instance doesn’t make this private key available to you (the vendor).

Upload the CSR to your internal infrastructure and then perform the signing process.

#### Sign the Csr

Download an MDM Vendor CSR Signing Certificate from the [`Apple Developer Portal`](https://developer.apple.comhttps://developer.apple.com/help/account/certificates/mdm-vendor-csr-signing-certificate). Then create a script to sign the customer’s CSR by following these instructions:
...
> **Note**:  Don’t share the private key from your device management (MDM) signing certificate with anyone, including your customers or resellers of your solution. Ensure that the process of signing the CSR takes place within your internal infrastructure and isn’t accessible to customers.
...
- The WWDR intermediate certificate, which is available from [`https://www.apple.com/certificateauthority/AppleWWDRCAG3.cer`](https://developer.apple.comhttps://www.apple.com/certificateauthority/AppleWWDRCAG3.cer).
- The Apple root certificate, which is available from [`http://www.apple.com/appleca/AppleIncRootCertificate.cer`](https://developer.apple.comhttp://www.apple.com/appleca/AppleIncRootCertificate.cer)
...
*[View on Apple Developer](https://developer.apple.com/documentation/devicemanagement/setting-up-push-notifications-for-your-device-magagement-customers)*