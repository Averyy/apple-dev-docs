...
- `Allowed` (boolean): If `true`, access is granted; otherwise, the process doesn’t have access. The user isn’t prompted and can’t change this value. > **Note**:  Every payload needs to include either `Authorization` or `Allowed`, but not both.
...
- `CodeRequirement` (string) *(required)*: Obtained via the command `codesign -display -r -`.
- `Comment` (string): Not used.
- `Identifier` (string) *(required)*: The bundle ID or installation path of the binary. > **Note**:  This value is case-sensitive.
