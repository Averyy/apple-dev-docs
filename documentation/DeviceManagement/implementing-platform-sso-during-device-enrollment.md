...
The device management service verifies the authorization token (step 15), and if that succeeds, it responds to the request by returning the device management [`MDM`](mdm.md) enrollment profile (step 16), which the device uses to enroll (step 17).
