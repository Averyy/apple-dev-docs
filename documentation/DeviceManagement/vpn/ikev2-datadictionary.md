...
- `EnforceStrictAlgorithmSelection` (integer): If set to `1`, the device doesn’t allow DES, 3DES, and Diffie-Hellman groups less than 14. Also the device requires the encryption algorithm specified for the IKE SA to be at least as cryptographically strong as the algorithm specified for the child SA. The device rejects this profile payload if these requirements aren’t met. Available: iOS 18.5+ | iPadOS 18.5+ | macOS 15.5+ | tvOS 18.5+ | visionOS 2.5+ | watchOS 11.5+
...
- `ProviderDesignatedRequirement` (string): If the VPN provider uses a system extension, then this field is required. Available: macOS 10.15+ | tvOS 17+ | watchOS 10+
