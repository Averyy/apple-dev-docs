...
Make sure that the Apple servers — listed in [`Allow Apple IP Addresses for Domain Verification`](setting-up-your-server#Allow-Apple-IP-Addresses-for-Domain-Verification.md) — can access the URL you used to validate the merchant domain. If the URL is behind a proxy or redirect the Apple servers won’t be able to access it. The URL may be similar to `https://yourdomain.com/.well-known/apple-developer-merchantid-domain-association`.
