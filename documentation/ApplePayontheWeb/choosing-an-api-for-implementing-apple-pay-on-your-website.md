...
Safari supports two APIs for implementing payment requests: [`Apple Pay JS API`](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api), and the W3C [`Payment Request API`](https://developer.apple.com/documentation/apple_pay_on_the_web/payment_request_api). Both APIs present the same Apple Pay payment sheet on Safari, and offer nearly the same user experience.
...
Use [`Apple Pay JS API`](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api) if you depend on any of its unique features:
...
- You can retry if an error occurs after the user authorizes payment. With [`Payment Request API`](https://developer.apple.com/documentation/apple_pay_on_the_web/payment_request_api), the user must restart their transaction.
...
- You can adjust prices or other terms of a sale for customers using your affiliated card. For example, you might provide free shipping when customers use your cobranded VISA credit card.
 ...
 You can request a phonetic name in [`Apple Pay JS API`](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api) only.

##### Features of Payment Request Api

Use [`Payment Request API`](https://developer.apple.com/documentation/apple_pay_on_the_web/payment_request_api) for these benefits:
...
When implementing [`Payment Request API`](https://developer.apple.com/documentation/apple_pay_on_the_web/payment_request_api), consider also implementing [`Apple Pay JS API`](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api) as a fallback for customers whose devices run an older operating system version.
