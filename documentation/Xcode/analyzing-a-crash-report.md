...
> ❗ **Important**: Always analyze a fully symbolicated crash report generated by the operating system. See [`Determine if a crash report is symbolicated`](adding-identifiable-symbol-names-to-a-crash-report#Determine-if-a-crash-report-is-symbolicated.md) to verify your crash report is fully symbolicated.

Always analyze a fully symbolicated crash report generated by the operating system. See [`Determine if a crash report is symbolicated`](adding-identifiable-symbol-names-to-a-crash-report#Determine-if-a-crash-report-is-symbolicated.md) to verify your crash report is fully symbolicated.
...
If you’re unfamiliar with a specific field or its value in this section of crash report, consult [`Header`](examining-the-fields-in-a-crash-report#Header.md).
...
If you’re unfamiliar with a field in this section, see [`Exception information`](examining-the-fields-in-a-crash-report#Exception-information.md).
...
If you’re unfamiliar with a field in this section, see [`Diagnostic messages`](examining-the-fields-in-a-crash-report#Diagnostic-messages.md).

##### Read the Backtraces

The backtraces in a crash report show the exact methods executing at the time of the crash—see [`Backtraces`](examining-the-fields-in-a-crash-report#Backtraces.md) for a breakdown of what each column in this section means. As a starting point, look at the crashed thread, as well as the `Last Exception Backtrace` if one is present. Answer these questions about the backtrace:
...
You can frequently determine the purpose of a thread based on the bottom frames in the thread’s backtrace. An app’s main thread has [`NSApplicationMain(_:_:)`](https://developer.apple.com/documentation/AppKit/NSApplicationMain(_:_:)) or [`UIApplicationMain(_:_:_:_:)`](https://developer.apple.com/documentation/UIKit/UIApplicationMain(_:_:_:_:)-1yub7) in the bottom frames. Threads created through the [`Dispatch`](https://developer.apple.com/documentation/Dispatch) framework have `start_wqthread` in the bottom frames. As you look closer at the crashed thread’s backtrace, consider whether your app appears in a coherent state that matches your expectations about how your app functions:
...
- If your code uses an API where you provide a [`DispatchQueue`](https://developer.apple.com/documentation/Dispatch/DispatchQueue) to the API, does the crash report show you’re using the queue you expect?
...
- Do any of the other threads help indicate what state the app is in? For example, if you see threads with frames from the [`Contacts`](https://developer.apple.com/documentation/Contacts) framework and you only access contacts in one part of your app, you can focus your investigation on the crash to that part of the app.
...
##### Understand the Crashed Threads Registers
...
- If you use `atos` to symbolicate the address in the link register, what function is it? Does that function jump to other code through function pointers? [`Symbolicate the crash report with the command line`](adding-identifiable-symbol-names-to-a-crash-report#Symbolicate-the-crash-report-with-the-command-line.md) describes how to use `atos`.

[`Identify the type of memory access that caused the issue`](investigating-memory-access-crashes#Identify-the-type-of-memory-access-that-caused-the-issue.md) describes how to use these questions to diagnose a memory access crash.
...
For the meaning of each column in this section, see [`Binary images`](examining-the-fields-in-a-crash-report#Binary-images.md).
