...
- `token`: A UUID that identifies the export operation. Pass   if your app presents the export sheet through a person’s interaction with your app’s UI. If the system launches your app with the   activity, pass the token you retrieve from the activity’s info dictionary using the  .
