...
- A `Regex` created from a string using the [`init(_:)`](one/init(_:).md) initializer has an output type of `AnyRegexOutput`, whether it has captures or not.
...
*[View on Apple Developer](https://developer.apple.com/documentation/regexbuilder/one/regexoutput)*