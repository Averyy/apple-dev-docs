# IOSurfaceLockOptions

**Framework**: IOSurface  
**Kind**: struct

**Availability**:
- iOS 11.0+
- iPadOS 11.0+
- Mac Catalyst 13.0+
- macOS 10.6+
- tvOS 11.0+
- visionOS 1.0+

## Declaration

```swift
struct IOSurfaceLockOptions
```

## Topics

### Initializers
- [init(rawValue: UInt32)](iosurfacelockoptions/init(rawvalue:).md)
### Options
- [static var avoidSync: IOSurfaceLockOptions](iosurfacelockoptions/avoidsync.md)
- [static var readOnly: IOSurfaceLockOptions](iosurfacelockoptions/readonly.md)

## Relationships

### Conforms To
- [BitwiseCopyable](../Swift/BitwiseCopyable.md)
- [Equatable](../Swift/Equatable.md)
- [ExpressibleByArrayLiteral](../Swift/ExpressibleByArrayLiteral.md)
- [OptionSet](../Swift/OptionSet.md)
- [RawRepresentable](../Swift/RawRepresentable.md)
- [Sendable](../Swift/Sendable.md)
- [SendableMetatype](../Swift/SendableMetatype.md)
- [SetAlgebra](../Swift/SetAlgebra.md)

## See Also

- [struct IOSurfacePropertyKey](iosurfacepropertykey.md)
- [struct IOSurfacePurgeabilityState](iosurfacepurgeabilitystate.md)


---

*[View on Apple Developer](https://developer.apple.com/documentation/iosurface/iosurfacelockoptions)*