...
1. Call [`acquirePresentmentAssertion()`](CredentialSession/acquirePresentmentAssertion().md) before showing any proprietary payment UI. 2.Call [`relinquish()`](CredentialSession/PresentmentIntentAssertion/relinquish().md) to stop using the assertion before invoking the transaction method.
2. Invoke [`configuration()`](CredentialSession/configuration().md) to start a transactionTask.
3. Perform the transaction with the transactionTask.
4. Call [`invalidate()`](CredentialTransaction/Configuration/invalidate().md) after presenting the credential.
5. Optionally, call [`acquirePresentmentAssertion()`](CredentialSession/acquirePresentmentAssertion().md)  to finish up any proprietary payment UI.
6. Call [`relinquish()`](CredentialSession/PresentmentIntentAssertion/relinquish().md) again to end use of the assertion.
