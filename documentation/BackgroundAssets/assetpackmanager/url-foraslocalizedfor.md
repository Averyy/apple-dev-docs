...
> ⚠️ **Warning**:  This method is less efficient than [`contents(at:asLocalizedFor:options:)`](assetpackmanager/contents(at:aslocalizedfor:options:).md) and [`descriptor(for:asLocalizedFor:)`](assetpackmanager/descriptor(for:aslocalizedfor:).md); use those methods when possible. In particular, this method shouldn’t be used to get the URL to the root of the shared asset-pack namespace. Don’t use this method to block the main thread.
