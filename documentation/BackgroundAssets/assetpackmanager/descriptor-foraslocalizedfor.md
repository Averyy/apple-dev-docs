...
Use this method if you need low-level access to the file descriptor. If you don’t, then use [`contents(at:asLocalizedFor:options:)`](assetpackmanager/contents(at:aslocalizedfor:options:).md) instead. It’s your responsibility to close the file descriptor when you’re done using it.
