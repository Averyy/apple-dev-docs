...
- `shouldUpdate`: Whether to require that the latest version be available locally. When   is passed to this parameter, the method will wait for the update (if there indeed is one available) to be downloaded before returning. When   is passed, the method won’t check for updates and won’t attempt to download any.
