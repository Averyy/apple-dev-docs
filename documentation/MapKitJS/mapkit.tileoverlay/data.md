...
The `data` object holds a dictionary of custom properties to fill the [`urlTemplate`](mapkit.tileoverlay/urltemplate.md).

- If the [`urlTemplate`](mapkit.tileoverlay/urltemplate.md) is a callback, MapKit JS passes the entire data object as a parameter.
- If the [`urlTemplate`](mapkit.tileoverlay/urltemplate.md) is a string, `data` key names should match up with the custom placeholder names. MapKit JS ignores the keys that don’t exist in the URL template string.
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkitjs/mapkit.tileoverlay/data)*