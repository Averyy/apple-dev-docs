...
This option only applies to search autocomplete. For example, if you send  to [`autocomplete`](mapkit.search/autocomplete.md), the search returns only . If you send  to [`autocomplete`](mapkit.search/autocomplete.md) while `includeQueries` is `false`, the search returns only addresses and points of interest (given that the setting for those options is `true`) related to  and not necessarily related to .
