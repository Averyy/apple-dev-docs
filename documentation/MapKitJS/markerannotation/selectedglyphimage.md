...
Set the size of the selected glyph image to 40 x 40 pixels. Create glyph images as template images — a monochrome image with opacity, if needed — so that MapKit JS can apply the [`glyphColor`](markerannotation/glyphcolor.md) to tint the image. MapKit JS scales glyph images to fit in the balloon. If you don’t set [`selectedGlyphImage`](markerannotation/selectedglyphimage.md), the framework uses [`glyphImage`](markerannotation/glyphimage.md) when the user selects the marker. The default image is a pin.
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkitjs/markerannotation/selectedglyphimage)*