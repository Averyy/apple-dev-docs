...
Pinch-to-zoom with a trackpad requires browser touch event support. You can zoom the map programmatically when zoom isn’t in an enabled state by changing the [`region`](mapkit.map/region.md) or [`visibleMapRect`](mapkit.map/visiblemaprect.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkitjs/mapkit.map/iszoomenabled)*