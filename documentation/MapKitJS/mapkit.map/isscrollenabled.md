...
You can programmatically cause the map to scroll when scrolling isn’t in an enabled state by changing the map’s [`center`](mapkit.map/center.md) and [`region`](mapkit.map/region.md) coordinates.
...
*[View on Apple Developer](https://developer.apple.com/documentation/mapkitjs/mapkit.map/isscrollenabled)*