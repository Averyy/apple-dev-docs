...
This constant is a Boolean attribute. If it is present in the attributes array,  specifies that AGL should consider only those renderers   that have a back color buffer the full size of the drawable object (regardless of window visibility). AGL guarantees that the back buffer contents will be valid after a call to the [`aglSwapBuffers`](https://developer.apple.comhttps://developer.apple.com/library/archive/#id(F15030)) function. You can pass this constant to the function [`AGL`](agl-collection.md).
...
- [var AGL_ROBUST: Int32](agl_robust.md)
...
*[View on Apple Developer](https://developer.apple.com/documentation/agl/agl_backing_store)*