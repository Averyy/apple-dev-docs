...
This method is equivalent to [`path(forResource:ofType:inDirectory:)`](bundle/path(forresource:oftype:indirectory:)-swift.method.md), except that only nonlocalized resources and those in the language-specific `.lproj` directory specified by `localizationName` are searched.
...
- `localizationName`: The language ID for of the localization. This parameter should correspond to the name of one of the bundle’s language-specific resource directories without the   extension.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/bundle/path(forresource:oftype:indirectory:forlocalization:))*