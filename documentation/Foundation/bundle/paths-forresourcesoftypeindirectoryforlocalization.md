...
This method is equivalent to [`paths(forResourcesOfType:inDirectory:)`](bundle/paths(forresourcesoftype:indirectory:)-swift.method.md), except that only nonlocalized resources and those in the language-specific `.lproj` directory specified by `localizationName` are searched.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/bundle/paths(forresourcesoftype:indirectory:forlocalization:))*