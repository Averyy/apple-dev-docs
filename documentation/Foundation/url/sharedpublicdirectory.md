...
In iOS, this directory is within the app’s sandbox directory. In macOS, it’s within the app’s sandbox directory for sandboxed apps, or in the current user’s home directory `(~/Public)` if the app isn’t sandboxed.

This computed property is equivalent to calling [`NSSearchPathForDirectoriesInDomains(_:_:_:)`](nssearchpathfordirectoriesindomains(_:_:_:).md) with the [`FileManager.SearchPathDirectory.sharedPublicDirectory`](filemanager/searchpathdirectory/sharedpublicdirectory.md) parameter.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/url/sharedpublicdirectory)*