...
Converts the little-endian formatted value in `x` to the current endian format and returns the resulting value. If it is necessary to swap the bytes of `x`, this function calls [`NSSwapFloat(_:)`](nsswapfloat(_:).md) to perform the swap.

## See Also

- [func NSSwapBigFloatToHost(NSSwappedFloat) -> Float](nsswapbigfloattohost(_:).md)
  Swaps the bytes of a number.
- [func NSSwapHostFloatToLittle(Float) -> NSSwappedFloat](nsswaphostfloattolittle(_:).md)
  Swaps the bytes of a number.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsswaplittlefloattohost(_:))*