...
Converts the little-endian formatted value in `x` to the current endian format and returns the resulting value. If it is necessary to swap the bytes of `x`, this function calls [`NSSwapShort(_:)`](nsswapshort(_:).md) to perform the swap.

## See Also

- [func NSSwapHostShortToLittle(UInt16) -> UInt16](nsswaphostshorttolittle(_:).md)
  Swaps the bytes of a number.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsswaplittleshorttohost(_:))*