...
Since at present only `file://` URLs are supported, there is no difference between this method and [`write(toFile:options:)`](nsdata/write(tofile:options:).md), except for the type of the first argument.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsdata/write(to:options:))*