...
The implementation of your overriding method must match the implementation of your [`encode(_:)`](nscoder/encode(_:)-1qd1e.md) method. For example, a typical [`encode(_:)`](nscoder/encode(_:)-1qd1e.md) method encodes the number of bytes of data followed by the bytes themselves. Your override of this method must read the number of bytes, create an `NSData` object of the appropriate size, and decode the bytes into the new `NSData` object.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/decodedata())*