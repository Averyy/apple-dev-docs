...
`NSCoder`’s implementation invokes [`decodeValue(ofObjCType:at:)`](nscoder/decodevalue(ofobjctype:at:).md) to decode the object data.

Subclasses may need to override this method if they override any of the corresponding `encode…Object` methods. For example, if an object was encoded conditionally using the [`encodeConditionalObject(_:)`](nscoder/encodeconditionalobject(_:).md) method, this method needs to check whether the object had actually been encoded.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/decodeobject())*