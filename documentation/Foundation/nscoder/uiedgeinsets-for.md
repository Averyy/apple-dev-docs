...
In general, you should use this function only to convert strings that were previously created using the [`string(for:)`](nscoder/string(for:)-26b4z.md) function.

## Parameters

- `string`: A string whose contents are of the form “{ ,  ,  ,  }”, where  ,  ,  ,   are the floating-point component values of the   structure. An example of a valid string is @”{3.0,8.0,3.0,5.0}”. The string is not localized, so items are always separated with a comma.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/uiedgeinsets(for:))*