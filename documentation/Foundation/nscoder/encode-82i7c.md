...
`NSCoder`’s implementation invokes [`encodeValue(ofObjCType:at:)`](nscoder/encodevalue(ofobjctype:at:).md) to encode `size`.

This method must be matched by a subsequent [`decodeSize()`](nscoder/decodesize().md) message.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/encode(_:)-82i7c)*