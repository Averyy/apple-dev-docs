...
`NSCoder`’s implementation invokes [`encodeValue(ofObjCType:at:)`](nscoder/encodevalue(ofobjctype:at:).md) to encode `point`.

This method must be matched by a subsequent [`decodePoint()`](nscoder/decodepoint().md) message.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/encode(_:)-75jv4)*