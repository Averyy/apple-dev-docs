...
The encoded object is decoded with the [`decodeObject(forKey:)`](nscoder/decodeobject(forkey:).md) method. If `objv` was never encoded unconditionally, [`decodeObject(forKey:)`](nscoder/decodeobject(forkey:).md) returns `nil` in place of `objv`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/encodeconditionalobject(_:forkey:))*