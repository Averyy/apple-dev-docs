...
`NSCoder`’s implementation invokes [`encodeValue(ofObjCType:at:)`](nscoder/encodevalue(ofobjctype:at:).md) to encode `rect`.

This method must be matched by a subsequent [`decodeRect()`](nscoder/decoderect().md) message.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nscoder/encode(_:)-3c1wz)*