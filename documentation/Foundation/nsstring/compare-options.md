...
This method is equivalent to invoking [`compare(_:options:range:)`](nsstring/compare(_:options:range:).md) with a given mask as the options and the receiver’s full extent as the range.

> ❗ **Important**:  When working with text that’s presented to the user, use the [`localizedStandardCompare(_:)`](nsstring/localizedstandardcompare(_:).md) instead, or use the [`compare(_:options:range:locale:)`](nsstring/compare(_:options:range:locale:).md) method, passing the user’s locale.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsstring/compare(_:options:))*