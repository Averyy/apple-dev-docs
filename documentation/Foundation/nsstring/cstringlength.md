...
Raises if the receiver can’t be represented in the default C-string encoding without loss of information. You can also use [`canBeConverted(to:)`](nsstring/canbeconverted(to:).md) to check whether a string can be losslessly converted to the default C-string encoding. If it can’t, use [`lossyCString()`](nsstring/lossycstring().md) to get a C-string representation with some loss of information, then check its length explicitly using the ANSI function `strlen()`.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsstring/cstringlength())*