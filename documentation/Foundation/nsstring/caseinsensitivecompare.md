...
This method is the equivalent of invoking [`compare(_:options:)`](nsstring/compare(_:options:).md) with [`caseInsensitive`](nsstring/compareoptions/caseinsensitive.md) as the only option.

> ❗ **Important**:  When working with text that’s presented to the user, use the [`localizedCaseInsensitiveCompare(_:)`](nsstring/localizedcaseinsensitivecompare(_:).md) method instead.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsstring/caseinsensitivecompare(_:))*