...
Invokes [`range(of:options:range:)`](nsstring/range(of:options:range:).md) with the options specified by `mask` and the entire extent of the receiver as the range.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsstring/range(of:options:))*