...
Note that this method only works with file paths (not, for example, string representations of URLs). The method does not check the filesystem for the existence of the path (use [`fileExists(atPath:)`](filemanager/fileexists(atpath:).md) or similar methods in [`FileManager`](filemanager.md) for that task).
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsstring/isabsolutepath)*