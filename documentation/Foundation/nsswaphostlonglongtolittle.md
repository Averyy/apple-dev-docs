...
Converts the value in `x`, specified in the current endian format, to little-endian format and returns the resulting value. If it is necessary to swap the bytes, this function calls [`NSSwapLongLong(_:)`](nsswaplonglong(_:).md) to perform the swap.

## See Also

- [func NSSwapLittleLongLongToHost(UInt64) -> UInt64](nsswaplittlelonglongtohost(_:).md)
  Swaps the bytes of a number.
- [func NSSwapHostLongLongToBig(UInt64) -> UInt64](nsswaphostlonglongtobig(_:).md)
  Swaps the bytes of a number.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsswaphostlonglongtolittle(_:))*