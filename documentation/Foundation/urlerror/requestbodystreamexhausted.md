...
This error applies to clients on iOS that send a `POST` request using a body stream but don’t implement the [`URLSessionTaskDelegate`](urlsessiontaskdelegate.md) method [`urlSession(_:task:needNewBodyStream:)`](urlsessiontaskdelegate/urlsession(_:task:neednewbodystream:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/urlerror/requestbodystreamexhausted)*