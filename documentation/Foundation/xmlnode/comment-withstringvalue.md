...
An [`XMLNode`](xmlnode.md) object representing an comment node ([`XMLNode.Kind.comment`](xmlnode/kind-swift.enum/comment.md)) containing the text `stringValue` or `nil` if the object couldn’t be created. If `stringValue` is `nil` or an empty string, a content-less comment node is returned (`<!--->`).
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/xmlnode/comment(withstringvalue:))*