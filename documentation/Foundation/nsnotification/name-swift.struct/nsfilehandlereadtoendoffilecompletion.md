...
It makes the data available to observers by putting it in the `userInfo` dictionary. To cause the posting of this notification, you must send either [`readToEndOfFileInBackgroundAndNotify()`](filehandle/readtoendoffileinbackgroundandnotify().md) or [`readToEndOfFileInBackgroundAndNotify(forModes:)`](filehandle/readtoendoffileinbackgroundandnotify(formodes:).md) to an appropriate `NSFileHandle` object.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsnotification/name-swift.struct/nsfilehandlereadtoendoffilecompletion)*