...
> ❗ **Important**:  The Swift overlay to the Foundation framework provides the [`Locale`](locale.md) structure, which bridges to the [`NSLocale`](nslocale.md) class. For more information about value types, see [`Working with Foundation Types`](https://developer.apple.com/documentation/swift/working-with-foundation-types).
