...
The property returns `nil` if the information isn’t available. The allocated size in bytes may include space that metadata uses. This can be less than the value that [`totalFileSize`](urlresourcevalues/totalfilesize.md) returns after if it’s a compressed resource.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/urlresourcevalues/totalfileallocatedsize)*