...
Because children of directory file wrappers are not read immediately by the [`init(url:options:)`](filewrapper/init(url:options:)-70161.md) method unless the `NSFileWrapperReadingImmediate` reading option is used, even a newly-created directory file wrapper might not have the same contents as the directory on disk. You can use this method to determine whether the file wrapper’s contents in memory need to be updated.
