...
Converts the little-endian formatted value in `x` to the current endian format and returns the resulting value. If it is necessary to swap the bytes, this function calls [`NSSwapLongLong(_:)`](nsswaplonglong(_:).md) to perform the swap.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsswaplittlelonglongtohost(_:))*