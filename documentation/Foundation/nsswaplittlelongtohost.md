...
Converts the little-endian formatted value in `x` to the current endian format and returns the resulting value. If it is necessary to swap the bytes of x, this function calls [`NSSwapLong(_:)`](nsswaplong(_:).md) to perform the swap.

## See Also

- [func NSSwapLong(UInt) -> UInt](nsswaplong(_:).md)
  Swaps the bytes of a number.
- [func NSSwapHostLongToLittle(UInt) -> UInt](nsswaphostlongtolittle(_:).md)
  Swaps the bytes of a number.
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsswaplittlelongtohost(_:))*