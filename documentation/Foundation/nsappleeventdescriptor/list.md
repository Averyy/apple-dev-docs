...
A list descriptor is a descriptor whose data consists of one or more descriptors. You can add items to the list by calling [`insert(_:at:)`](nsappleeventdescriptor/insert(_:at:).md) or remove them with [`remove(at:)`](nsappleeventdescriptor/remove(at:).md).

Invoking this method is equivalent to allocating an instance of `NSAppleEventDescriptor` and invoking [`init(listDescriptor:)`](nsappleeventdescriptor/init(listdescriptor:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsappleeventdescriptor/list())*