...
This impacts clients on iOS that send a `POST` request using a body stream but do not implement the [`URLSessionTaskDelegate`](urlsessiontaskdelegate.md) delegate method [`urlSession(_:task:needNewBodyStream:)`](urlsessiontaskdelegate/urlsession(_:task:neednewbodystream:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/nsurlerrorrequestbodystreamexhausted-swift.var)*