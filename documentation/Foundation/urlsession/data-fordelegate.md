...
Use this method to wait until the session finishes transferring data and receive it in a single [`Data`](data.md) instance. To process the bytes as the session receives them, use [`bytes(for:delegate:)`](urlsession/bytes(for:delegate:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/foundation/urlsession/data(for:delegate:))*