...
- **A trust.** When you have a trust object, as described in [`Evaluating a Trust and Parsing the Result`](evaluating-a-trust-and-parsing-the-result.md), you can extract the associated certificate’s public key with a call to [`SecTrustCopyPublicKey(_:)`](sectrustcopypublickey(_:).md):
...
- **Another key.** When you have a private key, you can calculate the associated public key with the [`SecKeyCopyPublicKey(_:)`](seckeycopypublickey(_:).md) function:
