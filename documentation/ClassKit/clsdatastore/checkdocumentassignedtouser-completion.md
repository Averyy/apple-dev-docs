# checkDocumentAssignedToUser(_:completion:)
...
```swift
func isDocumentAssignedToUser(_ documentURL: URL) async throws -> Bool
```
...
```None
           ...
           The completion handler's `submittable` parameter will be `YES` when:
           ...
           The completion handler's `submittable` parameter will be `NO` when:
...
[[CLSDataStore shared] checkDocumentAssignedToUser:documentURL completion:^(BOOL submittable, NSError * _Nullable error) { dispatch_async(dispatch_get_main_queue(), ^{ if (error) { NSLog(@“Error checking assignment status: %@”, error); return; }
...
*[View on Apple Developer](https://developer.apple.com/documentation/classkit/clsdatastore/checkdocumentassignedtouser(_:completion:))*