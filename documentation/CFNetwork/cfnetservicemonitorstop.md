...
- `error`: Pointer to a   structure or  . For synchronous monitors, set the   field of this structure to the non-zero value you want to be set in the   structure when   returns. Note that when it returns,   returns   If the monitor was started asynchronously, set the   field to the non-zero value you want the monitor’s callback to receive when it is called. If this parameter is  , default values for the   structure are used: the domain is set to   and the error code is set to  .
