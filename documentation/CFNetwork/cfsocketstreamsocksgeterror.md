...
Calling [`CFSocketStreamSOCKSGetError(_:)`](cfsocketstreamsocksgeterror(_:).md) returns the error code itself, which must be interpreted in the context of the result of a call to [`CFSocketStreamSOCKSGetErrorSubdomain(_:)`](cfsocketstreamsocksgeterrorsubdomain(_:).md). Possible return values (beyond subdomain-specific values such as client versions and HTTP error codes) are listed in [`Secure Sockets (SOCKS) Errors`](1518266-secure-sockets-socks-errors.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/cfnetwork/cfsocketstreamsocksgeterror(_:))*