...
As a side effect when you call this function, Core Graphics assigns an appropriate initial value to the stroke color, based on the color space you specify. To change this value, call [`setStrokeColor(_:)`](cgcontext/setstrokecolor(_:)-4pd8p.md). Note that the preferred API is now [`setStrokeColor(_:)`](cgcontext/setstrokecolor(_:)-1sskg.md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/coregraphics/cgcontext/setstrokecolorspace(_:))*