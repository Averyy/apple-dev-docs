...
When hardware mirroring is enabled, the contents of a single framebuffer are rendered in all displays in the hardware mirroring set. All drawing operations are directed to the primary display in the set—see [`CGDisplayPrimaryDisplay(_:)`](cgdisplayprimarydisplay(_:).md).

For more information about display mirroring, see [`CGConfigureDisplayMirrorOfDisplay(_:_:_:)`](cgconfiguredisplaymirrorofdisplay(_:_:_:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/coregraphics/cgdisplayisinhwmirrorset(_:))*