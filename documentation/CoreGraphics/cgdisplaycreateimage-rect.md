...
An image containing the contents of the specified rectangle. If the display ID is invalid, the return value is `NULL`. In Objective-C, you’re responsible for releasing the image created by calling [`CGImageRelease`](cgimagerelease.md).

#### Discussion

The actual rectangle used is the rectangle returned by [`CGRectIntegral(_:)`](cgrectintegral(_:).md) with `rect` as a parameter.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coregraphics/cgdisplaycreateimage(_:rect:))*