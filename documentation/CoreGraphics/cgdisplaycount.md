...
Quartz uses `CGDisplayCount` to represent a count of either the current or the maximum number of displays in a display list. For example, see the function [`CGGetActiveDisplayList(_:_:_:)`](cggetactivedisplaylist(_:_:_:).md).
...
*[View on Apple Developer](https://developer.apple.com/documentation/coregraphics/cgdisplaycount)*