...
If this key is not included in the dictionary, then the entire display is streamed. The value must be created using the [`CGRectCreateDictionaryRepresentation(_:)`](cgrectcreatedictionaryrepresentation(_:).md) function. The rectangle is specified in points in the display’s logical coordinate system.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coregraphics/cgdisplaystream/sourcerect)*