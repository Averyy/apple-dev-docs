...
Register for SSID change notifications by sending the [`startMonitoringEvent(with:)`](cwwificlient/startmonitoringevent(with:).md) message to a Wi-Fi client object with the [`CWEventType.ssidDidChange`](cweventtype/ssiddidchange.md) event type.

Use the Wi-Fi interface’s [`ssidData()`](cwinterface/ssiddata().md) or [`ssid()`](cwinterface/ssid().md) method to query the current SSID.
...
*[View on Apple Developer](https://developer.apple.com/documentation/corewlan/cweventdelegate/ssiddidchangeforwifiinterface(withname:))*