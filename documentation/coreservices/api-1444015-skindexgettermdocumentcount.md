...
If you want to know in which documents a term appears across multiple indexes, call this function separately on each index. Before querying each index, get the index-specific term ID using [`SKIndexGetTermIDForTermString(_:_:)`](1448558-skindexgettermidfortermstring.md).

To ensure that this function takes into account document URL objects (of type [`SKDocument`](skdocument.md)) recently added to indexes, call [`SKIndexFlush(_:)`](1450667-skindexflush.md) on each index before calling this function.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coreservices/1444015-skindexgettermdocumentcount)*