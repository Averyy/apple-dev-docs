...
> **Note**: This protocol handles the complete open and close lifecycle for items. When a volume conforms to `FSVolumeDataCacheHandler`, FSKit routes every open and close operation through this protocol’s [`open(_:modes:cacheMode:context:replyHandler:)`](fsvolume/datacachehandler/open(_:modes:cachemode:context:replyhandler:).md) and [`close(_:context:replyHandler:)`](fsvolume/datacachehandler/close(_:context:replyhandler:).md) methods. A volume that adopts this protocol doesn’t need to also conform to [`FSVolume.OpenCloseHandler`](fsvolume/openclosehandler.md).
