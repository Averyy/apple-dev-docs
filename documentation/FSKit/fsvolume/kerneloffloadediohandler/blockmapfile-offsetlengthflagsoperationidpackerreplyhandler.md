...
- result: A block or closure to indicate success or failure. If mapping succeeds, pass an instance of [`FSBlockmapResult`](fsblockmapresult.md) containing the volume’s updated free space, along with a `nil` error. If mapping fails, pass the relevant error as the second parameter; FSKit ignores the [`FSBlockmapResult`](fsblockmapresult.md) instance in this case. For an `async` Swift implementation, there’s no reply handler; simply return the result instance or throw an error.
