# exportFlattened(to:)

**Framework**: USDKit  
**Kind**: method

Exports the stage as a flattened USD file.

**Availability**:
- iOS 27.0+ (Beta)
- iPadOS 27.0+ (Beta)
- Mac Catalyst 27.0+ (Beta)
- macOS 27.0+ (Beta)
- tvOS 27.0+ (Beta)
- visionOS 27.0+ (Beta)

## Declaration

```swift
func exportFlattened(to url: URL) throws
```

#### Discussion

Composes all layers into a single layer and writes it to the specified location. The output format is determined by the file extension.

> ❗ **Important**: Flattening is destructive. Layer structure, composition arcs, and variant selections are resolved into the output. To preserve layer structure, use `exportPackage(to:options:)` instead.

Throws an error if the extension is not `.usda`, `.usdc`, or `.usd`.

## See Also

- [func exportPackage(to: URL, options: USDStage.ExportOptions) throws](usdstage-4sfi1/exportpackage(to:options:)-6s2wk.md)
  Packages the stage into a USDZ archive.
- [func exportPackage(to: FilePath, options: USDStage.ExportOptions) throws](usdstage-4sfi1/exportpackage(to:options:)-2x7yr.md)
  Packages the stage into a USDZ archive.
- [func exportFlattened(to: FilePath) throws](usdstage-4sfi1/exportflattened(to:)-6717d.md)
  Exports the stage as a flattened USD file.
- [USDStage.ExportOptions](usdstage-4sfi1/exportoptions.md)
  Options for packaging a stage into a USDZ file.


---

*[View on Apple Developer](https://developer.apple.com/documentation/usdkit/usdstage-4sfi1/exportflattened(to:)-98kpc)*