...
> ❗ **Important**:  This method is not supported in iOS, and not recommended in macOS 10.11 and later. Supply and use a block-based ROI callback instead using the [`apply(extent:roiCallback:arguments:)`](cikernel/apply(extent:roicallback:arguments:).md) method. The discussion below applies only to OS X v10.10 and earlier.
...
- `userInfo` is the object associated with the `kCIApplyOptionUserInfo` option when the kernel is applied to its arguments (with the [`apply(_:arguments:options:)`](cifilter-swift.class/apply(_:arguments:options:).md) method of a [`CIFilter`](cifilter-swift.class.md) object using the kernel). The `userInfo` is important because instance variables can’t be used by the defining class. Instance variables must be passed through the `userInfo` argument.
...
Alternatively, use the [`apply(extent:roiCallback:arguments:)`](cikernel/apply(extent:roicallback:arguments:).md) method to directly apply a kernel to create an output image, specifying the ROI callback as a block or closure.
...
*[View on Apple Developer](https://developer.apple.com/documentation/coreimage/cikernel/setroiselector(_:))*