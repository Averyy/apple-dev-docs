...
- `d`: The bitmap data for the image. This data must be premultiplied.
- `bpr`: The number of bytes per row.
- `size`: The dimensions of the image.
- `f`: The format and size of each pixel. You must supply a pixel format constant. See  .
- `cs`: The color space that the image is defined in. If this value is  , the image is not color matched. Pass   for images that don’t contain color data (such as elevation maps, normal vector maps, and sampled function tables).
...
*[View on Apple Developer](https://developer.apple.com/documentation/coreimage/ciimage/1547023-imagewithbitmapdata)*