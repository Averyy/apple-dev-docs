...
```swift
- (instancetype)initWithBitmapData:(NSData *)data bytesPerRow:(size_t)bytesPerRow size:(CGSize)size format:(CIFormat)format colorSpace:(CGColorSpaceRef)colorSpace;
```
...
- `d`: The bitmap data to use for the image. The data you supply must be premultiplied.
- `bpr`: The number of bytes per row.
- `size`: The size of the image data.
- `f`: A pixel format constant. See  .
- `c`: The color space that the image is defined in. It must be a Quartz 2D color space ( ). Pass   for images that don’t contain color data (such as elevation maps, normal vector maps, and sampled function tables).
...
*[View on Apple Developer](https://developer.apple.com/documentation/coreimage/ciimage/1437857-initwithbitmapdata)*