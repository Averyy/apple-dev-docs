...
Guest physical address for redistributor base aligned to byte value returned by [`hv_gic_get_redistributor_base_alignment(_:)`](hv_gic_get_redistributor_base_alignment(_:).md). The redistributor region contains redistributors for all vCPUs that the virtual machine supports.

## Parameters

- `config`: A GIC configuration object.
- `redistributor_base_address`: The guest’s physical address for the redistributor.
...
*[View on Apple Developer](https://developer.apple.com/documentation/hypervisor/hv_gic_config_set_redistributor_base(_:_:))*