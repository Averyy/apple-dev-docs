...
Virtual CPUs (vCPUs) in a virtual machine map to POSIX threads. Create a new vCPU for the current thread with [`hv_vcpu_create(_:_:)`](hv_vcpu_create(_:_:_:).md). The vCPU runs when the thread calls [`hv_vcpu_run(_:)`](hv_vcpu_run(_:).md).
...
- Create a virtual CPU with [`hv_vcpu_create(_:_:)`](hv_vcpu_create(_:_:_:).md).
