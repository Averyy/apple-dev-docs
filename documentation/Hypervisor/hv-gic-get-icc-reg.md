...
```swift
func hv_gic_get_icc_reg(_ vcpu: hv_vcpu_t, _ reg: hv_gic_icc_reg_t, _ value: UnsafeMutablePointer<UInt64>) -> hv_return_t
```
...
- `reg`: The value that represents the GIC ICC system register.
- `value`: A pointer to ICC register value that the framework writes to upon success.
...
*[View on Apple Developer](https://developer.apple.com/documentation/hypervisor/hv_gic_get_icc_reg(_:_:_:))*