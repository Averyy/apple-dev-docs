...
```swift
func hv_gic_get_ich_reg(_ vcpu: hv_vcpu_t, _ reg: hv_gic_ich_reg_t, _ value: UnsafeMutablePointer<UInt64>) -> hv_return_t
```
...
ICH registers are only available when exception level 2 (EL2) is in an enabled state, otherwise the method returns an error.
...
- `reg`: The value that represents the GIC ICH system register.
- `value`: The pointer to ICH register value that the frameworks writes to upon success.
...
*[View on Apple Developer](https://developer.apple.com/documentation/hypervisor/hv_gic_get_ich_reg(_:_:_:))*