...
> ❗ **Important**: A single [`NLEmbedding`](nlembedding.md) instance isn’t safe for concurrent use. Although its query methods, like [`vector(for:)`](nlembedding/vector(for:).md), look like read-only lookups, calling them on one instance from multiple threads or tasks at the same time can crash your app. Serialize access to a shared instance, or give each thread or task its own instance.
