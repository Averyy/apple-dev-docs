...
The data in this report details RoomCaptureSession usage and status.
...
| Room Capture Session Status | RoomCaptureSession stats including error type | `string` |

#### Glossary

| Dimension | Value | Definition |
| --- | --- | --- |
| Room Capture Session Status | success | Success without an error |
| Room Capture Session Status | exceedSceneSizeLimit | An error that indicates when the scene size grows past the framework’s limitations |
| Room Capture Session Status | worldTrackingFailure | An error that indicates an issue with the underlying ARKit session |
| Room Capture Session Status | invalidARConfiguration | An error that indicates when the ARKit session runs an unsupported configuration |
| Room Capture Session Status | deviceTooHot | An error that indicates when the device thermal metrics surpass the framework’s limitations |
| Room Capture Session Status | deviceNotSupported | An error that indicates that the framework doesn’t support the user’s device |
| Room Capture Session Status | internalError | An error that indicates when the framework encounters an unexpected error case |
...
*[View on Apple Developer](https://developer.apple.com/documentation/analytics-reports/roomplan-usage)*