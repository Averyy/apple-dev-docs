...
The data in this report details the options configured for World Tracking.
...
| Automatic Image Scale Estimation | Whether automatic image scale estimation is enabled | `boolean` |
| Collaborative Session | Whether the collaborative session feature is enabled | `boolean` |
| HDR Environment Texturing | Whether the session uses HDR environment texturing | `boolean` |
| Environment Texturing Mode | The configured environment texturing mode | `integer` |
| AR Frame Semantics | The configured frame semantics | `integer` |
| Number Of Tracked Images | The maximum number of images tracked in parallel | `integer` |
| Plane Detection Mode | The configured plane detection mode | `integer` |
| Scene Reconstruction Mode | The configured scene reconstruction mode | `integer` |
| User Face Tracking | Whether user face tracking is enabled | `boolean` |
| App Clip Code Tracking | Whether App Clip Code Tracking is enabled | `boolean` |
...
*[View on Apple Developer](https://developer.apple.com/documentation/analytics-reports/arkit-world-tracking)*