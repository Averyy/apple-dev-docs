...
The data in this report contains aggregated details about interactions with live text. It includes the length of selections, the word counts, and the total words found in the image.
...
| Event Type | Whether the user interacts with live text via a selection change, copying, or selecting all | `string` |
| Rich Pasteboard | Whether the selected text has rich pasteboard content | `boolean` |
| Selection Length | The character count of the user’s selection | `integer` |
| Selection Word Count | The number of words in the user’s selection | `integer` |
| Total Text Length | The full length of all of the text found in the image | `integer` |

#### Glossary

| Dimension | Value | Definition |
| --- | --- | --- |
| Event Type | SelectionChanged | Triggered if a selection changes |
| Event Type | Copy | Triggered if copy is invoked |
| Event Type | SelectAll | Triggered if select all is invoked |
...
*[View on Apple Developer](https://developer.apple.com/documentation/analytics-reports/visionkit-live-text-usage)*