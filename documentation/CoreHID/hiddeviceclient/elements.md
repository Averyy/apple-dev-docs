...
Elements of interest can be taken from this list and passed to [`monitorNotifications(reportIDsToMonitor:elementsToMonitor:)`](hiddeviceclient/monitornotifications(reportidstomonitor:elementstomonitor:).md) to receive notifications using [`HIDDeviceClient.Notification.elementUpdates(values:)`](hiddeviceclient/notification/elementupdates(values:).md) when updates to the elements are received from the device. Elements can also be used in a [`HIDDeviceClient.RequestElementUpdate`](hiddeviceclient/requestelementupdate.md) to request the latest data as desired.
...
*[View on Apple Developer](https://developer.apple.com/documentation/corehid/hiddeviceclient/elements)*