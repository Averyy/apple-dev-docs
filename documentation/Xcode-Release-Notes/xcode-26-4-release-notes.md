...
Xcode 26.4 beta includes Swift 6.2.3 and SDKs for iOS 26.4, iPadOS 26.4, tvOS 26.4, macOS 26.4, and visionOS 26.4. Xcode 26.4 beta supports on-device debugging in iOS 15 and later, tvOS 15 and later, watchOS 8 and later, and visionOS. Xcode 26.4 beta requires a Mac running macOS Tahoe 26.2 or later.
...
- Fixed: MCP servers configured in Codex’s config.toml file are overwritten by Xcode.  (169570663)
...
- When an XCTest class has the property `continueAfterFailure` set to false, a failure in an test method, `setUp`, or `tearDown` that is also declared `async` will skip any remaining retries for the current test. For example, if an affected test uses the “retry on failure” repetition mode, it will not attempt to re-run the test after failing.  (108565878)  In your test plan, set “Relaunch Tests for Each Repetition” to on.
- Instances of `UIImage` cannot be attached to tests when testing a Mac Catalyst app.  (168320788)  Use the `UIImage.cgImage` property to get the underlying `CGImage` and attach it instead.
- When an issue occurs in a Swift Testing test in a detached task or background thread, it may be recorded as a warning instead of as an error.  (170161483)  Set the environment variable `"SWIFT_TESTING_XCTEST_INTEROP_MODE"` to `"none"` in your test plan.
- When using Xcode for Apple silicon, tests which use Swift Testing may crash at launch when using a Rosetta run destination.  (170347005)  Use Xcode Universal.
